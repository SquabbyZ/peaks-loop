 ...
 * `peaks solo context-now --enforce-job-mode`.
 *
...
function runContextNow(args: readonly string[], env: NodeJS.ProcessEnv): CliResult {
  try {
    const stdout = execFileSync('node', [BIN, 'solo', 'context-now', ...args], {
...
describe('peaks solo context-now (v3.1.2) AC-15 job-mode action field', () => {
  test('AC-15a: ratio=0.87 with --enforce-job-mode → action=auto-compact-now', () => {
    ...
    expect(env.command).toBe('solo.context-now');
