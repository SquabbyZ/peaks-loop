...
async function loadGc() {
  ...
  for (const name of ['setPresenceLease', 'gcStalePresenceLeases'] as const)
...
describe('presence lease GC integration', () => {
  it('TC-SM-06: both age predicates are required for removal. RD §8. Pass criterion: assert.equal(result.removed, 1) and assert.equal(result.retained, 2).', async () => {
    const root = await mkdtemp(join(tmpdir(), 'peaks-presence-gc-'));
    projects.push(root);
    const service = await loadGc();
    const result = await service.gcStalePresenceLeases({
      projectRoot: root,
      now: '2026-08-03T10:00:00.000Z',
      leases: [
        stale,
        { ...stale, workflowId: 'young', startedAt: '2026-08-02T20:00:00.000Z' },
        { ...stale, workflowId: 'fresh-beat', lastHeartbeat: '2026-08-03T09:30:00.000Z' }
      ]
    });
  ...
  it('runs the same stale sweep before presence:set. RD §3. Pass criterion: assert.equal(result.gc.removed, 1) before assert.equal(result.lease.status, "preparing").', async () => {
    const root = await mkdtemp(join(tmpdir(), 'peaks-presence-set-gc-'));
    projects.push(root);
    const service = await loadGc();
    const result = await service.setPresenceLease({
      projectRoot: root,
      sessionId: 'session',
      callerId: 'caller',
      workflowId: 'fresh',
      graphRef: 'graphs/fresh.json',
      skill: 'peaks-code',
      now: '2026-08-03T10:00:00.000Z',
      staleLeases: [stale]
    });
  ...
  it('workspace-init trigger invokes the same GC service before binding a new lease. RD §3. Pass criterion: assert.equal(result.trigger, "workspace-init") and assert.equal(result.removed, 1).', async () => {
    const service = await loadGc();
    const result = await service.gcStalePresenceLeases({
      projectRoot: 'workspace-init-project',
  ...
  it('manual GC is explicit and does not treat missing graph evidence as live work. RD §4. Pass criterion: assert.equal(result.inFlightBatch, false) and assert.equal(result.warnings[0].code, "PEAKS_GRAPH_REF_BROKEN").', async () => {
    const root = await mkdtemp(join(tmpdir(), 'peaks-presence-manual-gc-'));
    projects.push(root);
    const service = await loadGc();
    const result = await service.gcStalePresenceLeases({
      projectRoot: root,
      trigger: 'manual',
      now: '2026-08-03T10:00:00.000Z',
      leases: [{ ...stale, graphRef: 'graphs/missing.json' }]
    });
