/**
 * v3.1.1 Step 0.8 — `peaks solo detect-job` / `peaks solo read-job-shape`
 * CLI integration tests.
 *
 * Strategy: spawn `node bin/peaks.js solo detect-job ... --json` against
...
function runCli(args: readonly string[], cwd: string): RunResult {
  try {
    const stdout = execFileSync('node', [BIN, 'solo', 'detect-job', ...args], {
      cwd,
...
function runCliRead(args: readonly string[], cwd: string): RunResult {
  try {
    const stdout = execFileSync('node', [BIN, 'solo', 'read-job-shape', ...args], {
      cwd,
      stdio: ['ignore', 'pipe', 'pipe'],
      timeout: BIN_TIMEOUT_MS
    }).toString('utf8');
...
describe('peaks solo detect-job / read-job-shape (v3.1.1 Step 0.8)', () => {
  test('happy path: writes the file, prints ok envelope, second call without --force fails with JOB_SHAPE_ALREADY_DECIDED', () => {
    ...
    expect(env1.command).toBe('solo.detect-job');
  ...
  test('read-job-shape after a successful detect returns the same record', () => {
    ...
    expect(read.command).toBe('solo.read-job-shape');
