/**
 * v3.1.2 Step 0.8 — integration test for `peaks solo gate-step-08`.
 *
 * Spawns `node bin/peaks.js solo gate-step-08` as a real child process
...
function runCli(args: readonly string[], cwd: string, stdinText?: string): RunResult {
  try {
    const stdout = execFileSync('node', [BIN, 'solo', 'gate-step-08', ...args], {
      cwd,
...
describe('peaks solo gate-step-08 (v3.1.2) hook integration', () => {
  test('AC-1: isJob=true with progress.json → exit 0 + Next: slice #N+1 of M', () => {
    ...
    expect(env.command).toBe('solo.gate-step-08');
  ...
  test('AC-3: no decision + backup-regex prompt match → exit 2 + BLOCKED stderr', () => {
    ...
    expect(r.stderr).toMatch(/peaks solo detect-job/);
