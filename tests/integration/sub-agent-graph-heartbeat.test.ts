...
type HeartbeatApi = AnyRecord & {
  heartbeat: (input: AnyRecord) => Promise<AnyRecord> | AnyRecord;
...
vi.mock('node:fs', async () => {
  ...
  return {
    ...
    writeFileSync: (...args: unknown[]) =>
      __fsMocks.writeFileSync?.(...args) ??
      actual.writeFileSync(...(args as Parameters<typeof actual.writeFileSync>))
  };
});

async function api(): Promise<HeartbeatApi> {
  const module =
    (await import('../../src/cli/commands/heartbeat-commands.js')) as unknown as AnyRecord;
...
describe('sub-agent graph heartbeat', () => {
  it('TC-AP-05: first heartbeat is the only dispatched → running transition. RD §8. Pass criterion: assert.equal(first.status, "running") and assert.equal(second.status, "running").', async () => {
    ...
    const first = await service.heartbeat({
      dispatchRef: 'dispatch/one.json',
      graphNodeId: 'node-one',
      status: 'dispatched',
  ...
  it('TC-SM-02: terminal-failed dispatch becomes lost with sub-agent-crashed. RD §3. Pass criterion: assert.equal(result.status, "lost") and assert.equal(result.terminalReason, "sub-agent-crashed").', async () => {
    ...
    const result = await service.markLost({
      dispatchRef: 'dispatch/failed.json',
      graphNodeId: 'node-failed',
      status: 'running',
      reason: 'sub-agent-crashed'
    });
