...
import { P0_JOURNEY_IDS, type CapabilityBaselineRow } from '~/src/services/capability-baseline/types';
...
const CONTRACTS: ReadonlyArray<GuardContract> = ROWS.map((row) => ({
  ...
  execute: async () => ({ journeyId: row.journeyId, contract: 'workflow-trace', status: 'pass', artifactPath: 'a' }),
...
function allPass(): ReadonlyArray<GuardRunResult> {
  return CONTRACTS.map((c) => ({ journeyId: c.journeyId, contract: c.kind, status: 'pass' as const, artifactPath: 'a' }));
}

const FULL_SUMMARY = { pass: P0_JOURNEY_IDS.length, fail: 0, skipped: 0, total: P0_JOURNEY_IDS.length, results: allPass() };
...
describe('runAudit (independent eval)', () => {
  it('never reports consistent when the scorer is a stub', async () => {
    ...
    const r = await runAudit({
      projectRoot: proj, sessionId: 'a', journeyId: 'J01', scorerMode: 'stub',
      baselineRows: ROWS, contracts: CONTRACTS, guardSummary: FULL_SUMMARY
    });
  ...
  it('returns verdict=consistent when the live checker is clean and the guards agree', async () => {
    proj = mkdtempSync(join(tmpdir(), 'cbl-aud-'));
    const r = await runAudit({
      projectRoot: proj, sessionId: 'a', journeyId: 'J01', scorerMode: 'live',
      baselineRows: ROWS, contracts: CONTRACTS, guardSummary: FULL_SUMMARY
    });
  ...
  it('scores one dimension per guard result', async () => {
    ...
    const r = await runAudit({
      projectRoot: proj, sessionId: 'a', journeyId: 'J01', scorerMode: 'live',
      baselineRows: ROWS, contracts: CONTRACTS,
      guardSummary: {
        pass: P0_JOURNEY_IDS.length - 1,
        fail: 1,
        skipped: 0,
        total: P0_JOURNEY_IDS.length,
        results: allPass().map((g) => (g.journeyId === 'J02'
