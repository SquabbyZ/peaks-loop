...
function results(statusFor: (j: CapabilityBaselineRow['journeyId']) => GuardRunResult['status']): ReadonlyArray<GuardRunResult> {
  return CONTRACTS.map((c) => ({ journeyId: c.journeyId, contract: c.kind, status: statusFor(c.journeyId), artifactPath: 'a' }));
}
...
describe('5th-dim injection', () => {
  it('a drifted audit forces the 5th dim to fail', async () => {
    ...
    const audit = await runAudit({
      projectRoot: proj, sessionId: 'i', journeyId: 'J01', scorerMode: 'live',
  ...
  it('a narrowed observation set drifts even when every contract that ran passed', async () => {
    ...
    expect(audit.verdict).toBe('drifted');
    expect(audit.findings?.map((f) => [f.code, f.journeyId])).toEqual([['OBSERVATION_INCOMPLETE', 'J09']]);
  });
});
