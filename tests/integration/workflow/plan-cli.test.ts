...
describe('peaks workflow plan * — integration (gated on PEAKS_BUILD_AVAILABLE=1)', () => {
  ...
  it_build(
    'T-101: workflow plan read security returns the expected envelope (exists:false on first run)',
    async () => {
      const r = await runPeaks(
        [
          'workflow',
          'plan',
          'read',
      ...
      expect(json.data.path).toBe(
        resolve(join(projectRoot, '.peaks', '_runtime', sessionId, 'qa', 'security-test-plan.md'))
      );
    }
  );

  it_build(
    'T-102: workflow plan read perf returns the expected envelope (exists:false on first run)',
    async () => {
      const r = await runPeaks(
        [
          'workflow',
      ...
      expect(json.data.exists).toBe(false);
      expect(json.data.path).toBe(
        resolve(join(projectRoot, '.peaks', '_runtime', sessionId, 'qa', 'perf-baseline.md'))
      );
    }
  );

  it_build(
    'T-103: workflow plan refresh security --apply writes file and is idempotent',
    async () => {
      const r1 = await runPeaks(
        [
          'workflow',
          'plan',
          'refresh',
  ...
  it_build(
    'T-105: workflow plan detect-trigger returns triggered:false on a no-change repo',
    async () => {
      const r = await runPeaks(
        [
          'workflow',
  ...
  it_build(
    'T-105b: workflow plan detect-trigger with --refresh returns reason "manual-override"',
    async () => {
      const r = await runPeaks(
        [
          'workflow',
