...
export function writeApprovedTechArtifacts(artifactWorkspace: string, sessionId: string): void {
  // Slice 2026-06-23-audit-5th-p1: stage under the canonical change-id
  // scope dir (`.peaks/_runtime/change/<sessionId>/rd/architecture/`),
  // matching where `tech-service.ts`'s `getTechStatus` reads from. The
  // old top-level `.peaks/_runtime/<sessionId>/` is a SKILL.md 2.8.3 hard-ban
  // violation — see `change-scope-service.ts` for the canonical
  // location.
  const architectureRoot = join(getSessionDir(artifactWorkspace, sessionId), 'rd', 'architecture');
...
export function writeResumeArtifacts(artifactWorkspace: string, sessionId: string, goal = 'Resume autonomous RD planning from artifacts'): void {
  // Slice 2026-06-23-audit-5th-p1: route under
  // `.peaks/_runtime/change/<sessionId>/...` (see `getSessionDir`).
  const changeRoot = getSessionDir(artifactWorkspace, sessionId);
  const artifacts = new Map([
    [join(changeRoot, 'prd', 'autonomous-goal-package.json'), JSON.stringify({ sessionId, artifactType: 'goal-package', status: 'ready', goal, doneCondition: 'all acceptance criteria pass', resumeCondition: 'checkpoint verified', acceptanceCriteria: ['validation evidence exists'] })],
    [join(changeRoot, 'rd', 'swarm', 'autonomous-rd-plan.json'), JSON.stringify({ sessionId, artifactType: 'rd-plan', status: 'ready', workerQueueStatus: 'ready', taskCount: 3, reducerRequired: true })],
    [join(changeRoot, 'rd', 'swarm', 'checkpoints', 'checkpoint-1.json'), JSON.stringify({ sessionId, artifactType: 'checkpoint', status: 'ready', checkpointId: 'checkpoint-1', createdAt: '2026-05-17T00:00:00.000Z', workerQueueState: { pending: 0, completed: 3 }, validationRefs: ['validation-details.md'] })],
