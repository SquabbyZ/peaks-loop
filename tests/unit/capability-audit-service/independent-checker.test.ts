...
import { P0_JOURNEY_IDS, type CapabilityBaselineRow, type JourneyId } from '~/src/services/capability-baseline/types';
...
const CONTRACTS: ReadonlyArray<GuardContract> = ROWS.map((r) => ({
  ...
  execute: async () => ({ journeyId: r.journeyId, contract: 'workflow-trace', status: 'pass', artifactPath: 'a' }),
  evidence: { kind: 'workflow-trace', artifact: 'a' }
}));

function results(ids: ReadonlyArray<JourneyId>): ReadonlyArray<GuardRunResult> {
  return ids.map((j) => ({ journeyId: j, contract: 'workflow-trace', status: 'pass', artifactPath: 'a' }));
}
...
describe('runIndependentCheck', () => {
  it('is consistent when the frozen set, the observation set and the bindings all agree', () => {
    ...
    const r = runIndependentCheck({ projectRoot: proj, baselineRows: ROWS, contracts: CONTRACTS, guardResults: ALL });
  ...
  it('drifts when a journey is missing from the observation set', () => {
    ...
    const r = runIndependentCheck({
      projectRoot: proj, baselineRows: ROWS, contracts: CONTRACTS,
      guardResults: ALL.filter((g) => g.journeyId !== 'J09')
    });
    expect(r.verdict).toBe('drifted');
    expect(r.findings.map((f) => [f.code, f.journeyId])).toEqual([['OBSERVATION_INCOMPLETE', 'J09']]);
  });
  ...
  it('drifts when an observation is not a frozen journey', () => {
    proj = mkdtempSync(join(tmpdir(), 'cbl-chk-'));
    const r = runIndependentCheck({
      projectRoot: proj, baselineRows: ROWS.filter((x) => x.journeyId !== 'J09'), contracts: CONTRACTS,
      guardResults: ALL
    });
  ...
  it('drifts when a frozen sourceFiles entry is gone from disk', () => {
    ...
    const r = runIndependentCheck({ projectRoot: proj, baselineRows: withBinding, contracts: CONTRACTS, guardResults: ALL });
    expect(r.verdict).toBe('drifted');
    expect(r.findings.map((f) => [f.code, f.journeyId, f.detail.includes('src/gone.ts')]))
      .toEqual([['SOURCE_FILE_MISSING', 'J01', true]]);
  });
  ...
  it('reports how many frozen invariants are armed without drifting on the rest', () => {
    ...
    const r = runIndependentCheck({ projectRoot: proj, baselineRows: ROWS, contracts: halfArmed, guardResults: ALL });
  ...
  it('does not read the guard statuses — a failing contract is the runner verdict input, not this one', () => {
    ...
    const r = runIndependentCheck({ projectRoot: proj, baselineRows: ROWS, contracts: CONTRACTS, guardResults: failing });
