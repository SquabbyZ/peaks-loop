 ...
 *
 * Verifies the integration between `peaks code should-pause --step
...
describe('peaks code should-pause × presence staleness — AC-2', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  test('3. Step 1 + fresh presence (same outer) → NOT stale-presence; mode-driven path applies', async () => {
    ...
    try {
      ...
      // (defect #1 fix from 2026-06-28-code-mode-bypass-fix).
      const base = shouldPauseAtGate({ mode: 'full-auto', step: 'step-1-mode-select' });
