 ...
 *   - `isCodeMode` / `isHardFloorCategory` type guards
 */
...
const NON_AUTO_MODES: readonly CodeMode[] = ['assisted', 'strict'];
const AUTO_MODES: readonly CodeMode[] = ['full-auto', 'swarm'];

describe('isCodeMode / isHardFloorCategory', () => {
  test('isCodeMode accepts all 4 declared modes', () => {
    for (const m of CODE_MODES) {
      expect(isCodeMode(m)).toBe(true);
    }
  });

  test('isCodeMode rejects unknown strings', () => {
    expect(isCodeMode('semi-auto')).toBe(false);
...
describe('shouldPauseAtGate — 4 modes × 14 GATED_STEPS (56 cases)', () => {
  // Slice 2026-06-28-code-mode-bypass-fix (defect #1): hard-pause
