...
import type { EvidenceBrief } from "../../../../src/services/crystallization/index.js";
import { z } from "zod";
...
describe("CrystallizationService", () => {
  it("rejects pre-run crystallization (AC-4 / RL-2)", () => {
    // Intentional invalid pre-run fixture: the service must reject it,
    // so the literal types are wider than CrystallizationTaskState by
    // design. Cast to a permissive shape so tsc does not reject the
    // (intentionally bad) inputs that the test is meant to surface.
    const task = {
      task_id: "t1",
      task_status: "running",
      gates_passed: false,
