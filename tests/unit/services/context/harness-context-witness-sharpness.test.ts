...
declareDimensions(
  ...
  [
    {
      dim: 'render',
      reason: 'No output shape is asserted here; the one-way sentence and the CLI envelope live in harness-context-witness.test.ts and code-context-now-witness.test.ts.',
    },
    {
      dim: 'integration',
      reason: 'Every case calls the comparison directly — no fs, subprocess, env, network or clock boundary is crossed.',
    },
    {
      dim: 'a11y',
      reason: 'No human-visible text, exit code or structured error message is produced by a pure comparison call.',
    },
  ],
);
          ...
          for (const gap of gaps) {
            for (let skew = -0.02; skew <= 0.08; skew += 0.0005) {
              ...
              if (comparison.verdict === 'agree') {
                masked.push(`ratio=${ratio.toFixed(2)} gap=${(gap * 100).toFixed(1)}% skew=${skew.toFixed(4)}`);
              }
            }
          }
        }
