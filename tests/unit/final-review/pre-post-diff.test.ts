...
describe('producePrePostDiff — a computable baseline', () => {
  it(
    'writes the artifact with before/after counts, deltas, and the added/removed export names',
  ...
  it(
    'feeds the dimension a real pre-post-diff evidence item so it can actually pass',
    { timeout: SUBPROCESS_TEST_TIMEOUT_MS },
    async () => {
...
describe('producePrePostDiff — an uncomputable baseline', () => {
  it(
    'reports why, writes no artifact, and keeps the dimension inconclusive in a real git work tree',
    { timeout: SUBPROCESS_TEST_TIMEOUT_MS },
    async () => {
...
describe('producePrePostDiff — no drift', () => {
  it(
    'says so explicitly instead of writing an empty file or a silent pass',
    { timeout: SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
...
describe('producePrePostDiff — a deletion is visible (F4) and never contradicted (F5)', () => {
  it(
    'reports a deleted module that carried no export line and no test case (F4)',
    { timeout: SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
  ...
  it(
    'reports a count drop whose names cannot be resolved instead of "nothing was removed" (F5)',
...
describe('producePrePostDiff — rewrites are not removals (F6)', () => {
  it(
    'does not report a re-export list that merely grew',
    { timeout: HEAVY_SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
      const root = makeRepo({
  ...
  it(
    'does not report an `it(` -> `test.each(` rewrite as removed cases',
    { timeout: HEAVY_SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
      const root = makeRepo({
  ...
  it(
    'does not report a moved test file as a removed one',
    { timeout: HEAVY_SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
...
describe('producePrePostDiff — the case count is symmetric (F10)', () => {
  it(
    'counts modifier spellings and a commented line the same way on both sides',
    { timeout: SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
...
describe('producePrePostDiff — base resolution and empty ranges (F7 / F8)', () => {
  it(
    'resolves origin/master when the default branch is not `main` (F7)',
    { timeout: HEAVY_SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
      const root = makeRepo({ 'src/a.ts': 'export const alpha = 1;\n' });
      const baseline = git(root, ['rev-parse', 'HEAD']).trim();
  ...
  it(
    'reports an empty range as unavailable instead of NO STRUCTURAL DRIFT (F8)',
    { timeout: SUBPROCESS_TEST_TIMEOUT_MS },
    async () => {
      ...
      const probe = producePrePostDiff({
        projectRoot: root,
        sessionId: SESSION_ID,
        baseRef: 'HEAD'
      });
...
describe('producePrePostDiff — the timestamp is not drift (F9)', () => {
  it(
    'differs between two runs on exactly the `Generated:` line, and says so',
    { timeout: HEAVY_SUBPROCESS_TEST_TIMEOUT_MS },
    () => {
      const root = makeRepo({
        'src/a.ts': 'export const alpha = 1;\n',
      ...
      const second = producePrePostDiff({
        projectRoot: root,
        sessionId: SESSION_ID,
        baseRef: base
      });
