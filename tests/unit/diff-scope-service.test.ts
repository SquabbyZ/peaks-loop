...
describe('getDiffVsScope', () => {
  test('returns rd-not-found when RD artifact missing', async () => {
  ...
  test('reports patternsDeclared=false when RD has no scope bullets', async () => {
    const project = await makeGitRepo();
  ...
  test('classifies files matching in-scope patterns as in-scope', async () => {
    const project = await makeGitRepo();
  ...
  test('flags out-of-scope writes as violations', async () => {
    const project = await makeGitRepo();
    await seedRdWithScope(project, '2026-05-25-feat', [
      'In-scope:',
      '- src/services/login/**',
  ...
  test('flags files matching no pattern as unclassified', async () => {
    const project = await makeGitRepo();
    await seedRdWithScope(project, '2026-05-25-feat', [
      'In-scope:',
      '- src/services/login/**',
  ...
  test('auto-allows test files and .peaks artifacts without requiring an in-scope pattern', async () => {
    const project = await makeGitRepo();
    await seedRdWithScope(project, '2026-05-25-feat', [
      'In-scope:',
      '- src/services/login/**',
  ...
  test('handles bullets without explicit subheaders (treats all as in-scope)', async () => {
    const project = await makeGitRepo();
    await seedRdWithScope(project, '2026-05-25-feat', [
      '- src/services/login/**',
      '- src/api/login.ts',
  ...
  test('returns gitAvailable=false when project is not a git repo', async () => {
