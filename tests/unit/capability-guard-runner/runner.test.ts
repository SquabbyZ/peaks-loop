...
import { exitCodeForGuardSummary, runAllGuards, runGuard } from '~/src/services/capability-guard-runner/runner';
...
const ctx = (contract: GuardContract) => ({ projectRoot: REPO, sessionId: 's', contract, baselineInvariant: 'auto' });
...
describe("Scenario: capability-guard-runner/runner", () => {
  ...
  it("when invoked, should refuse an invariant that is not verbatim in the frozen baseline", async () => {
    const fabricated: GuardContract = { ...okContract, source: { baselineRow: 'J01', invariant: 'The user must type a CLI verb to proceed' } };
  ...
  it("when invoked, should refuse a contract bound to a journey absent from the baseline", async () => {
    const ghost: GuardContract = { ...okContract, source: { baselineRow: 'J99' as never, invariant: J01_INVARIANT } };
  ...
  it("when invoked, should record a throwing contract as a fail instead of aborting the run", async () => {
    const boom: GuardContract = { ...okContract, execute: async () => { throw new Error('probe exploded'); } };
