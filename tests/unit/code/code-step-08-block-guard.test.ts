 ...
 *   - `peaks code gate-step-08` (PreToolUse hook)
 *   - `peaks code emit-handoff` (size-fear ban)
...
describe('peaks-code Step 0.8 (v3.1.1) block-guard', () => {
  ...
  test('SKILL.md mentions `peaks code detect-job` (recorder instruction)', async () => {
    const body = await readFile(SKILL_MD, 'utf8');
    expect(body).toMatch(/peaks code detect-job/);
  });

  test('SKILL.md mentions `read-job-shape` (downstream gate)', async () => {
    const body = await readFile(SKILL_MD, 'utf8');
    // SKILL.md uses backtick-quoted `read-job-shape` (without the `peaks code` prefix)
    // in the runbook prose; match the substring as it actually appears.
    expect(body).toMatch(/read-job-shape/);
  });

  test('runbook.md references `peaks code detect-job` BEFORE `# After Step 7`', async () => {
    const body = await readFile(RUNBOOK_MD, 'utf8');
    const detectJobIdx = body.indexOf('peaks code detect-job');
    const afterStep7Idx = body.indexOf('# After Step 7');
    expect(detectJobIdx, 'runbook.md must reference `peaks code detect-job`').toBeGreaterThanOrEqual(0);
...
describe('peaks-code Step 0.8 (v3.1.2) mechanical gates — SKILL.md / runbook lock', () => {
  test('SKILL.md mentions `peaks code gate-step-08` (PreToolUse hook)', async () => {
    const body = await readFile(SKILL_MD, 'utf8');
    expect(body, 'SKILL.md must reference peaks code gate-step-08 (v3.1.2 PreToolUse hook)').toMatch(/peaks code gate-step-08/);
  });

  test('SKILL.md mentions `peaks code emit-handoff` (size-fear ban)', async () => {
    const body = await readFile(SKILL_MD, 'utf8');
    expect(body, 'SKILL.md must reference peaks code emit-handoff (v3.1.2 size-fear ban)').toMatch(/peaks code emit-handoff/);
  });
  ...
  test('runbook.md mentions `peaks code gate-step-08`', async () => {
    const body = await readFile(RUNBOOK_MD, 'utf8');
    expect(body, 'runbook.md must reference peaks code gate-step-08').toMatch(/peaks code gate-step-08/);
  });

  test('runbook.md mentions `peaks code emit-handoff`', async () => {
    const body = await readFile(RUNBOOK_MD, 'utf8');
    expect(body, 'runbook.md must reference peaks code emit-handoff').toMatch(/peaks code emit-handoff/);
  });
});
