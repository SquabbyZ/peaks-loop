...
declareDimensions('tests/unit/lint/session-path-swallow-census.test.ts', ['render', 'behavior', 'integration'], [
  { dim: 'a11y', reason: 'reads source with a parser; renders no human-visible text or exit code of its own' },
]);
...
const CENSUS: readonly Entry[] = [
  ...
  },
  {
    ...
    reason: 'tmp-file unlink is best effort, and `throw error` follows the catch — nothing is swallowed',
  },
  {
...
async function measuredCensus(): Promise<Array<{ file: string; rule: string; line: number }>> {
  ...
  const byKey = (a: { file: string; line: number }, b: { file: string; line: number }) =>
    (a.file + String(a.line).padStart(6, '0')).localeCompare(b.file + String(b.line).padStart(6, '0'));
  return out.sort(byKey);
}

const expectedCensus = () =>
  CENSUS.map((c) => ({ file: c.file, rule: c.rule, line: c.line })).sort(
    (a, b) => (a.file + String(a.line).padStart(6, '0')).localeCompare(b.file + String(b.line).padStart(6, '0')),
  );
...
describe('Scenario: behavior — the swallow census on the session-path surface', () => {
  ...
  it('when scanned, should find NO swallow at all in the frames that resolve a session id', async () => {
    ...
    for (const file of [
      ...
      const source = readFileSync(join(REPO_ROOT, file), 'utf8');
      expect(await analyze(source, join(REPO_ROOT, file)), `${file} must have zero swallowing frames`).toEqual([]);
    }
  });
});
...
describe('Scenario: render — the envelope the detector itself emits', () => {
  it('when the guard is asked about a known swallow, should report it at the right rule and line', async () => {
    ...
    expect(await analyze(source, 'control.ts')).toMatchObject([{ rule: 'catch-return-null', line: 4 }]);
  });
