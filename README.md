...
[![Sponsor](https://img.shields.io/badge/sponsor-❤-ff69b4)](./README.md#-赞助-peaks-cli)

**一个 CLI + 11 个工作流技能，把 LLM 的随意发挥变成可审计的工程流程。**

[安装](#-30-秒跑起来) · [5 分钟上手](#-5-分钟上手) · [技能家族](#-11-个技能家族) · [杀手锏：不可绕过的门禁](#-杀手锏不可绕过的门禁) · [Sponsor](#-赞助-peaks-cli)
...
peaks 的 11 个 `peaks-*` 技能同样发布到 [skills.sh](https://skills.sh) 注册表。任何装了 `npx skills` 的环境都能直接拉取：
...
浏览 [skills.sh](https://skills.sh) 看完整目录。技能和 `npm install -g peaks-cli` 是同一份内容——两条路径产物一致。
...
peaks-cli 是 **MIT 协议的开源项目**——你看到的每一行 skill / CLI / hook 都在 [SquabbyZ/peaks-cli](https://github.com/SquabbyZ/peaks-cli) 公开维护。维护团队是一支小队伍，我们一边做项目一边 dogfood 在自己的真实项目上（ice-cola / peaks-cli 自己 / 客户的工程改造）。

**赞助直接变成这些事的燃料**：

- 🛠️ **多 IDE 适配**——把 Trae / Codex / Cursor / Qoder 从"已注册"推进到"真实 dogfood"
- 🧪 **测试 + CI 投入**——每多一个跨 IDE 集成就多一份矩阵；当前 2800+ 测试 / tsc / vitest / lint 全部 green，但 matrix 在长
- 📚 **技能家族扩展**——`peaks-doc` / `peaks-i18n` / `peaks-perf` / `peaks-security` 等还在路线图
- 🤝 **社区运营**——issue 响应、sop 模板库、合作伙伴的 SOP 接入
- 🔒 **安全审计**——门禁机制是产品命脉，独立的安全 review 一年一次起步

**怎么赞助**：

- ⭐ **GitHub Sponsors**（首选）：[github.com/sponsors/SquabbyZ](https://github.com/sponsors/SquabbyZ) —— 一次性 / 月度 / 自定义金额
- 🎁 **Open Collective**：账号注册中（联系 maintainer 加进 README）
- 💼 **企业级支持 / 私有定制 SOP**：开 issue 标记 `sponsor:enterprise` 我们会跟进
- 🐛 **报告 bug / 提 PR / 写文档**：非现金贡献同样关键，merged PR 会出现在 release notes 的 "thanks to" 段落

**透明的赞助使用**：我们每季度在 [discussions](https://github.com/SquabbyZ/peaks-cli/discussions) 发一次"赞助都花哪儿了"——具体到 CI 分钟数 / 域名费 / 跨 IDE 测试机时。

> 企业批量部署、内部 SOP 模板库、定制培训：直接联系 maintainer。
