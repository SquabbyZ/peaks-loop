---
name: peaks-loop-when-adding-a-new-subcommand-check-for-existing-top-level-first
description: peaks-loop: when adding a new subcommand, check for existing top-level first
metadata:
  type: rule
  sourceArtifact: .peaks/_runtime/2026-06-10-session-6bcac7/txt/handoff-2026-06-10-fuzzy-matching-implementation.md
---

In peaks-loop, each top-level command (`peaks memory`, `peaks retrospective`, etc.) is registered in exactly one place. If a slice needs a new subcommand under an existing top-level, the new subcommand's `.action(...)` registration must be added to the file that already owns the top-level — NOT registered as a new top-level.
