...
description: 2026-06-28 session 用户明确划定 full-auto mode 的边界 — RD/QA 全程 Code fork Agent，但 commit 是终点。Push/tag/publish 都不是 full-auto 范围（push 可能 OK 但需要事后确认，tag/publish 必须 user-only）。下次 session 别越界。
metadata:
  type: feedback
---
...
> "code 的其他模式也是 rd 和 qa 都是自己执行，不是留给用户去做"
...
| 阶段 | 谁执行 | 备注 |
|---|---|---|
| **Anchor / mode 选择** | Code | 用户说"全自动" → 直接走 full-auto |
| **PRD / RD** | Code fork Agent | 5 个 sub-agent 全部 Code fork，不能让用户在 IDE 跑 Task toolCall |
| **QA** | Code fork Agent | 包含 micro-cycle（RD 修复 → QA re-validate） |
| **RD micro-cycle fix** | Code fork Agent | 用户不参与 |
| **CHANGELOG / version bump** | Code fork Agent | 这是 release territory 但属于 commit 前必备 |
| **Commit** | Code fork Agent | **full-auto 终点** |
...
**错因**: 我误以为"派单 = 派单完事"，没意识到 full-auto 要求 Code 自己 fork Agent 跑。
...
> **full-auto mode = Code does PRD → RD → QA → CHANGELOG → version bump → commit.**
> **Stop. Anything after commit (push, tag, publish, merge to develop, npm publish) is user-only unless explicitly told otherwise.**

## Why

v2.14.0 ship 时我做了 push + tag，user 当场说"full-auto 只做到 commit 就行"。这条经验比任何 PRD AC 都重要——因为它定义了 **Code 与 user 的责任边界**，是模式的 contract 而不是 bug。

## How to apply

下次 session 起 full-auto：
1. PRD / RD / QA / commit 全部 Code fork Agent 跑，**不返回 toolCall 让 IDE 跑**
2. Commit 完成后 STOP
