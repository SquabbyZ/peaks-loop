...
Slice #009 (PRD #002) added a new `peaks sub-agent dispatch <role> --prompt <text> --json` CLI primitive. **It is the IDE-agnostic primitive that the peaks-code / peaks-rd / peaks-qa / peaks-ui / peaks-txt SKILL.md compose to dispatch sub-agents. Users do NOT invoke this directly.**
...
The `peaks sub-agent dispatch` CLI clears that bar on grounds 2 (structured JSON envelope: `data.toolCall = { name, args }`) and is the natural machine primitive for "give me a tool-call descriptor I can invoke in my environment". The skill text in `skills/peaks-code/SKILL.md` (and the runbook / references) is the only place the LLM learns to call it. The CLI's `--help` text declares this explicitly:

> This command is the primitive that peaks-code / peaks-rd / peaks-qa / peaks-ui / peaks-txt SKILL.md compose to dispatch sub-agents. Users do not invoke this directly.
...
1. The SKILL.md text in peaks-code / peaks-rd / peaks-qa / peaks-ui (and any future Dispatcher) is the **only place** the LLM learns to call the CLI. Do not assume the LLM "knows" the CLI exists; document the call pattern explicitly.
2. The dispatch envelope's `instruction` field must keep the wording that names the SKILL.md contract (the reference doc `skills/peaks-code/references/sub-agent-dispatch.md`).
