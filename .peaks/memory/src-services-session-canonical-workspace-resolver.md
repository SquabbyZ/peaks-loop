...
**Risk surface:** High. A path-literal regression in this resolver (e.g. dropping `_runtime` from the join, or normalizing the root aggressively) would break every downstream write path. The `tests/unit/services/session/session-dir-canonical.test.ts` static scan guards against regressions in OTHER files, but a regression INSIDE the resolver itself is caught by the resolver-shape case in that same file, and more thoroughly by `tests/unit/services/session/get-session-dir.test.ts` (which pins the canonical shape, separator handling, and the negative "no legacy top-level layout" assertion).

**Corrected (2026-09-10, S5 rebuild).** The claim above that the static scan "guards against regressions in OTHER files" was false for part of the interval: that test file was **deliberately deleted in `f17aa377`** ("delete 559 legacy unit tests") and was not rebuilt until this slice. It was also wrong to attribute the resolver-shape coverage to "the existing 3 tests" — that count belonged to the deleted file; the live coverage now lives in the two files named above.

**Why:** Slice `005-session-runtime-dir-regression` (2026-06-07) introduced this resolver to consolidate 4 stragglers that were hard-coding the legacy `<root>/.peaks/<sid>/`. (That legacy path was originally written correctly here and in `getSessionDir.ts`'s docstring; commit `e09deae6`'s blind global `.peaks/<id>` → `.peaks/_runtime/<id>` swap rewrote both, including the negative mentions. Recovered from `git show 4775690e`.) The static-scan test makes future bypasses impossible to ship silently.
