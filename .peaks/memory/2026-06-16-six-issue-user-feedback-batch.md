...
**Plus the follow-up observation (#7):** "peaks-loop 在自己的项目里效果比实际消费项目好很多" — root cause: PRD#1 G5 was marked NG by the original RD. Created PRD#7 (`2026-06-16-peaks-solo-auto-scaffold`) which adds `detectMissingProjectStandards` + `--init-standards` flag + once-per-session dedup marker.
...
**Lesson 3 — peaks-loop project integrates better than consumer projects BY DESIGN, not by accident.** peaks-loop project was manually scaffolded (before peaks-loop 2.0 existed), and the dev actively maintains it. Consumer projects (`platform-rag-web`) were never bootstrapped. The fix is PRD#7 (`peaks-solo-auto-scaffold`): when a consumer project's `.claude/rules/{common,typescript}/` is empty, emit a diagnostic + offer `--init-standards`. Marker file `.peaks/_runtime/<sid>/.standards-checked` dedups to once-per-session.
