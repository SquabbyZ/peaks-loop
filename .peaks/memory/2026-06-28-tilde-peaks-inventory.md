...
| `logs/peaks-cli-2026-06-2{1..7}.log` (激进档 A-6) | log 累积 | 移到 `logs/archive/2026-Q2/` | `ls -R ~/.peaks/logs/` 显示归档结构 + 06-28 保留在 root |

### 保留项（用户运行时配置，**永远不要自动覆盖**）

- `~/.peaks/config.json` (423 B, version=**2.14.0**) —— 主配置
- `~/.peaks/workspaces.json` (73 B) —— peaks-cli 启动 schema 校验依赖
- `~/.peaks/logs/peaks-cli-2026-06-28.log` —— 当前 session 日志，保持可访问
...
| **P2** | ~~`logs/peaks-cli-2026-06-2{1..7}.log` 共 2.4 MB~~ | 默认 7 天保留是设计行为；磁盘不紧可不清理 | 已移到 `logs/archive/2026-Q2/` | ✅ ARCHIVED (2026-06-28 11:19) |
...
│   ├── peaks-cli-2026-06-28.log   148K  ← 当前 session
│   └── archive/
│       └── 2026-Q2/                ← 7 个归档 log（2.3 MB）
│           ├── peaks-cli-2026-06-21.log    96K
│           ├── peaks-cli-2026-06-22.log   176K
│           ├── peaks-cli-2026-06-23.log   148K
│           ├── peaks-cli-2026-06-24.log   624K
│           ├── peaks-cli-2026-06-25.log   780K
│           ├── peaks-cli-2026-06-26.log   260K
│           └── peaks-cli-2026-06-27.log   328K
...
- **`~/.peaks/` 是全局配置，不归 peaks-cli 仓库管**——任何清理都不会被 `git status` 看到，但会被 `peaks-companion` / `peaks-doctor` 启动时读取。
...
$ cd "C:/Users/smallMark/Desktop/peaks-cli" && pnpm vitest run tests/unit/doctor.test.ts 2>&1 | tail -5
...
# Step 3
$ ls -R ~/.peaks/logs/
peaks-cli-2026-06-28.log
archive/2026-Q2/:
peaks-cli-2026-06-21.log  peaks-cli-2026-06-22.log  peaks-cli-2026-06-23.log
peaks-cli-2026-06-24.log  peaks-cli-2026-06-25.log  peaks-cli-2026-06-26.log
peaks-cli-2026-06-27.log
```
...
`~/.peaks/workspaces/invalid-artifact/` 不是"清掉就完了"——它在 v2.14.0 ship 期间再次重生（mtime 09:41），证明这是 peaks-cli workspace init 的预期行为，不是 stale residue。
