...
| `node "…/services/hooks/write-gate.js"` (`.claude/settings.local.json` Write\|Edit\|MultiEdit matcher) | `PreToolUse` `Write\|Edit\|MultiEdit`         | `src/services/hooks/write-gate.js` (emitted by `src/services/workspace/claude-settings-template.ts`) | every file write; reads the path from the payload on **stdin** (`tool_input.file_path`, with `path` / `notebook_path` fallbacks), argv as fallback | exit 0 = allow **only** for a path under `.peaks/_runtime/`; exit 1 = fall through to gate (no stdout/stderr). Never returns 2. | `tests/unit/workspace/write-gate-decision-table.test.ts` (32 cases) + `tests/unit/workspace-init-hooks.test.ts` |
