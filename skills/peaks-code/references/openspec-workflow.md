# OpenSpec Lifecycle for Peaks Code

Peaks Code orchestrates RD, QA, and SC. When the target repository uses OpenSpec, Code must drive the full lifecycle through the Peaks CLI so each role works against the same stable surface.
...
peaks openspec validate →  Code gates RD output before slicing starts
...
peaks openspec archive  →  Code moves the change under changes/archive/<id>/
```
...
- `archive --apply` is the lifecycle terminator; Code only invokes it after QA acceptance and SC commit.

## Refactor workflow wiring

For `peaks-code refactor` runs against a repository with `openspec/`:
...
Code must not write `openspec/changes/**` or `~/.claude/settings.json` directly. Every mutation goes through the CLI commands above. The CLI returns stable envelopes; Code captures them as artifact links rather than re-explaining their content in the handoff.

---

# Step 0.5 — OpenSpec first-run opt-in (conditional)

> Body of `### Peaks-Loop Step 0.5`. After the workspace is anchored, before project scan, Code checks whether
