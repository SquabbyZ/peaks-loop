...
When the runner crosses the v2.13.0 auto-compact thresholds (0.80 auto-fire / 0.85 pre-compact / 0.95 red-line), `peaks session checkpoint` is NOT enough on its own — the LLM must also fire `peaks code auto-compact` (or wait for the in-band PreToolUse hook at ratio ≥ 0.95 to compact the runner itself). See:

- `peaks code auto-compact` — orchestrator that writes the convergence plan + dispatches the compact pathway
- `peaks code auto-compact` — the PreToolUse hook entrypoint fired by `.claude/settings.local.json` (slice 2026-07-02-auto-compact-zero-pause)
- `.peaks/memory/2026-06-27-auto-compact-design.md` — zero-human-intervention design rationale

The two surfaces compose: `peaks session checkpoint` persists the context state, `peaks code auto-compact` collapses the runner's window. The LLM fires both from the 0.80 auto-fire line up (that is the tier `peaks skill presence` reports first); the 0.85–0.95 band is the "already fired" zone, and the hook fires alone at ≥ 0.95.