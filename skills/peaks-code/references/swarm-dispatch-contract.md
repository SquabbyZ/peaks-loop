...
> fan-out rule** in `skills/peaks-code/SKILL.md` §"Peaks-Loop Default
...
- Smoke test: a full-auto peaks-code run on a `legacy-frontend` project with a UI-affecting PRD should produce `sc/swarm-plan.json` containing all three sub-agents and three corresponding artefacts in `ui/`, `rd/`, `qa/`.
