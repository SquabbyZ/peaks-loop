...
Code delegates layer-by-layer fan-out + join-barrier + cancel-on-fail
to **`runDag(dag, opts)`** in `src/services/code/dag-orchestrator.ts`
when a slice's dependency surface is a DAG. Code MUST NOT re-implement
...
```ts
import { runDag } from '../services/code/dag-orchestrator.js';
...
Code gets partial `DagRunResult`, treats slice as `return-to-rd`,
...
- Orchestrator: `src/services/code/dag-orchestrator.ts` (`runDag` +
