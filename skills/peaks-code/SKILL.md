...
**Probe primitive:** `peaks code context-now --json` — authoritative for `ratio` and for the `action` tier **from 0.85 up**. Do NOT use `peaks context check --prompt-size` (deprecated, will silently under-report ratio). Returns `{ ratio, action: 'ok' | 'soft-warn' | 'auto-compact-now' | 'red-line' }` — Code reads `action` and dispatches `peaks code auto-compact` on `auto-compact-now` or `red-line` without user confirmation. **It has no 0.80 tier** (see the tier table above): the `0.80 ≤ ratio < 0.85` band belongs to `peaks skill presence`'s `auto-fire`, and that probe runs every turn. **Read both probes and act on whichever fires first — the earlier of the two governs, so 0.80, not 0.85.**
...
| **D-010** | 11c | `peaks memory extract` returns `extractedCount: 0` despite `<!-- peaks-memory:start -->` existing | Block requires YAML frontmatter (`title:` + `kind:` + `---`) + closing `<!-- peaks-memory:end -->`. A marker without those fields, and a marker that is not the exact literal (e.g. with a `kind=…` written inside it), produce no writes and are both named in the envelope's `warnings[]` — read it before concluding there was no block | §Step 11c + 11d |
