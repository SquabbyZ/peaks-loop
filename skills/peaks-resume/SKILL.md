...
```bash
peaks skill presence:set peaks-resume --project <repo> --mode <mode> --gate startup
...
**If `fresh`**: no in-flight slice. Tell the user "No in-flight slice in this session." Then hand off to `peaks-code` for a fresh start (set presence back, then run `peaks workspace init --project <repo> --json` and continue with Step 1 of `peaks-code`).

**If `complete`**: the workflow is already done. Tell the user "This session is complete — your previous slice already shipped." Then suggest a fresh start (`peaks-code`) or close-out.

**If `resume:<gate>`**: surface a resume option via `AskUserQuestion`:

| Option | What it does |
|---|---|
| Resume from `<gate>` (Recommended) | Hand off to `peaks-code` with the gate already at the matching point. The main loop skips ahead; the existing artifacts are preserved as-is. |
...
## Step 3: Hand off to peaks-code

After the user confirms, **re-assert `peaks-code` presence** so the status header reads correctly for the rest of the run:

```bash
peaks skill presence:set peaks-code --project <repo> --mode <mode> --gate <matching-gate>
```

Then tell the user: "Resuming from `<gate>`. The main peaks-code skill will take over from there." and yield control. The user's next message will land in the `peaks-code` main loop, which will pick up the workflow from the matching gate.
...
- **Never write code or modify files in this skill.** This is a wrapper. The actual work (PRD, RD, QA, SC, TXT) is the main `peaks-code` skill's job.
