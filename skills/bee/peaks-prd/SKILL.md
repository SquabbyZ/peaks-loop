...
## Peaks-Loop Step 2.5 sub-step: best-practice scan handoff

When peaks-prd's `businessGoal` artifact transitions to `complete` + the sub-step has not
yet fired for this intent, peaks-prd MUST emit a sub-step handoff to Step 2.5 best-practice
scan BEFORE transitioning to Step 3 RD.

**Handoff contract:**
- Input: `businessGoal` (natural language) + `.peaks/project-scan/project-scan.md` (auto-generated by `peaks workspace init`)
- Output: best-practice scan artifact at `.peaks/_runtime/<sessionId>/best-practice/<requestId>.md`
- Trigger CLI: `peaks best-practice-scan --project <repo> --intent "<businessGoal>" --json`
- Status: `awaiting-user-ack` (3 valid user responses per spec §7: 接受 / 接受方案 N / 拒绝 + 原因)

**State machine:** peak-prd's state machine refuses `--allow-incomplete` style bypass for
this sub-step. Silent auto-accept is NOT allowed — user explicit ack is mandatory.

**Reference:** See `skills/peaks-code/SKILL.md` §Step 2.5 for the full sub-step contract.
