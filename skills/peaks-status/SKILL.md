...
```bash
peaks skill presence:set peaks-status --project <repo> --mode <mode> --gate startup
...
Active:    peaks-code (mode: full-auto, gate: qa-validation)
...
=== peaks-code status ===
Session: 2026-06-04-session-b60252
Active:  peaks-code (mode: full-auto, gate: startup)
Workspace: /Users/yuanyuan/.../peaks-loop/.peaks/2026-06-04-session-b60252
...
| Option | What it does |
|---|---|
| Continue from the current gate | Hand off to `peaks-code` with the gate already at the current state. Existing artifacts are preserved. |
...
- `peaks-resume` (P2.1) and `peaks-test` (P2.2) are the sibling wrappers. `peaks-resume` focuses on workflow-state introspection + resume decision; `peaks-status` is the broader 5-CLI snapshot. The two are complementary, not redundant.
