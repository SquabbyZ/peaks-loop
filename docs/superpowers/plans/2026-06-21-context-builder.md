...
export function createContextCommands(options: ContextCommandsOptions): Command {
  ...
  context
    .command('schema')
    .action(() => {
      process.stdout.write(JSON.stringify(ContextJsonSchema._def, null, 2) + '\n');
    });
...
import { headroomFetcher } from '../context/headroom-fetcher.js'; // written in Task 10
...
async function ensureContextForRd(goal: string, project: string, sid: string): Promise<void> {
  ...
  await buildContext({
    ...
    docBudgetTokens: 8000, out, fetcher: headroomFetcher,
  });
}
```
...
- A `DocFetcher` implementation that:
  - Checks local cache first (`.peaks/_runtime/<sid>/doc-cache/<dep>@<version>.md`)
...
describe('headroom-fetcher', () => {
  it('returns cached doc when version matches', async () => {
    // Stub: test by mocking the file read.
    const fetcher = createHeadroomFetcher({
      cacheDir: '/tmp/nonexistent-cache',
      remoteFetcher: vi.fn(),
    });
    // For v1, just verify the fetcher returns null on cache miss + remote null.
 ...
 * fetch; local cache (per-dep markdown) preferred when version matches.
 *
...
```bash
git add src/services/context/headroom-fetcher.ts tests/unit/services/context/headroom-fetcher.test.ts
git commit -m "feat(context): headroom-fetcher (production doc fetcher with cache)"
```
