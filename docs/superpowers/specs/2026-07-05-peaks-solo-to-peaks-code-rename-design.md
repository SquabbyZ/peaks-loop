...
**Date:** 2026-07-05
**Author:** SquabbyZ (via peaks-solo brainstorm session 2026-07-05)
**Affects:** peaks-solo (主技能), peaks-solo-resume, peaks-solo-status, peaks-solo-test, peaks-prd, peaks-rd, peaks-qa, peaks-ui, peaks-sc, peaks-txt (六个 role skill 全部下沉), `.peaks/skills/.system/bees/`, marketplace, hooks, tests, docs, ~110 memory files (历史只读).
...
- **HC-2 不计成本:** 不为减少工作量而妥协任何决策。例如:不为省事保留 peaks-solo 作为 alias、不为省事仅在描述里改字、不为省事把三个 sub-skill 留在 peaks-solo 树下。
...
> **⚠️ 反向设计:** 2026-07-04 的 `peaks-maker-dynamic-skill-sediment-design.md` §4.1.1 显式做了"保留 peaks-solo"决定(原话:"We honor the user's explicit request: '保留 peaks-solo 这个技能'")。本 spec 是对该决定的**主动覆盖**,不是新增。覆盖理由由 user 在 2026-07-05 brainstorm 中给出:**4.x 出版后整个项目不再是局限的代码开发,已经可以扩展到自定义的领域,后续会内置一些想 3.x 版本的 peaks-solo 定位一样的 loop engineering,趁现在使用的人只是几个,做一次调整,把 peaks-solo 更名为 peaks-code,是这个内置的 loop engineering 的名称和实际的内容及定位更加准确**。
...
> "这是个长痛和短痛的问题。开启 4.x 后整个项目不再是局限的代码开发,已经可以扩展到自定义的领域,后续会内置一些想 3.x 版本的 peaks-solo 定位一样的 loop engineering。趁现在使用的人只是几个,做一次调整。"

**短痛(本次承受):** 全量 id 重命名 + 全量测试 + marketplace + hooks + 文档同步。
**长痛(现在不做、未来持续):** peaks-solo 这个名字已经**直接误导用户**——它字面上暗示"solo = 单飞、孤立、零依赖",但实际 4.x 后的定位是"code-domain loop engineering orchestrator",与未来多个领域 loop engineering 并列存在。继续用 peaks-solo,用户会自然以为"peaks-solo 是唯一入口 / 其他领域只能靠 LLM 兜底",这与 4.x 多领域扩展的产品定位冲突。

### 1.2 与 2026-07-04 决定的兼容性

2026-07-04 spec §4.1.1 决定把 peaks-solo 保留为 `~/.peaks/skills/.system/bees/peaks-solo/` 的 system-stable 路径,**这条路径在本 spec 里继续保留**——只改 skill id 和 display name,不动 pool 物理路径。理由:pool 的 6-table 存储、retention、blob dedup 都已经过 dogfood,变更物理路径需要重新跑一遍回归。user 在 brainstorm 中明确选了"保持原路径不动"。

### 1.3 三个 sub-skill 必须一起处理

`peaks-solo-resume` / `peaks-solo-status` / `peaks-solo-test` 当前**语义上是 peaks-solo 的辅助技能**(resume 切片、查 session 状态、跑测试),但**功能上是通用 primitive**——任何 bee 都会有"恢复" / "查状态" / "跑测试"的需求。继续把它们放在 peaks-solo 名下会:

1. 误导用户以为"它们只服务于 peaks-solo";
2. 让未来新增的 bee 必须复制这三个技能,违反 DRY;
3. 模糊 "orchestrator(主技能) vs primitive(基础设施)" 的层次。

本次 spec 一并下沉并改名为通用 primitive:`peaks-solo-resume → peaks-resume`、`peaks-solo-status → peaks-status`、`peaks-solo-test → peaks-test`。
...
| 对象 | 改动 |
|---|---|
| `skills/peaks-solo/` | 目录改名 `skills/peaks-code/`,`SKILL.md` 的 frontmatter `name: peaks-solo` 改为 `name: peaks-code`,`description` 改为 "Code-domain loop engineering orchestrator for the Peaks-Loop skill family…" |
| `skills/peaks-solo-resume/` | 目录改名 `skills/peaks-resume/`,`SKILL.md` 的 `name: peaks-solo-resume` 改为 `name: peaks-resume`,description 改为通用 resume primitive |
| `skills/peaks-solo-status/` | 目录改名 `skills/peaks-status/`,`name` + description 同步改 |
| `skills/peaks-solo-test/` | 目录改名 `skills/peaks-test/`,`name` + description 同步改 |
| `~/.peaks/skills/.system/bees/peaks-solo/manifest.json` | manifest 的 `id: "peaks-solo"` 改为 `id: "peaks-code"`,`displayName` 改为 `Peaks Code`,保留物理路径 `.system/bees/peaks-solo/` 不动 |
...
| `.claude-plugin/marketplace.json` | 所有 `peaks-solo*` 条目 id 改名;六个 role skill 条目加 `"userInvocable": false` |
| `.claude/LOGGING.md` | 文中所有 `peaks-solo` 字面提及改为 `peaks-code`(代码片段、表格项、配置示例同步) |
| 仓库内所有源代码、测试、文档、scripts | 全量 `peaks-solo` 替换为 `peaks-code`,三个子技能全量替换;六个 role skill 的 slash trigger 字符串(`/peaks-rd` 等)从 user-facing 文档里移除或加 "(internal)" 标记 |
| `tests/unit/wrapper-skills.test.ts`、`tests/unit/doctor.test.ts`、`tests/fixtures/skills/pre-slim/peaks-solo.SKILL.md` | fixture / 测试名同步;新加 `tests/unit/skills-role-visibility.test.ts`(验证六个 role skill 都有 `visibility: internal`) |
| `.git/sdd/*.md`(历史 brief、report) | **不动**——历史交付物按 SHA 冻结 |
| `.peaks/memory/*.md`(110 个文件) | **不动**——memory 是历史沉淀,描述过去的事实。新记忆会用 peaks-code 这个名字,旧记忆里出现 peaks-solo 是合理的"历史快照" |
...
- **不动 pool 物理路径**:`.system/bees/peaks-solo/` 保持(只在 manifest 的 `id` 字段改 display)。
...
- **不写迁移脚本**:user 在 brainstorm 中明确说"我直接手动替换应该就行吧"。存量 `.peaks/_runtime/**/*.json` 里的 `skill: "peaks-solo"` 字段由 user 手动 sed,LLM 这次不提供 `--migrate` 自动命令。
...
| 旧名 | 新名 | 角色 |
|---|---|---|
| `peaks-solo` | `peaks-code` | code-domain loop engineering orchestrator(主技能) |
| `peaks-solo-resume` | `peaks-resume` | universal resume primitive |
| `peaks-solo-status` | `peaks-status` | universal status primitive |
| `peaks-solo-test` | `peaks-test` | universal test-runner primitive |
...
`~/.peaks/skills/.system/bees/peaks-solo/manifest.json`(物理路径不变):
...
- 历史 `.peaks/_runtime/active-skill.json`(若存在 `skill: "peaks-solo"`)由 user 手动 sed 替换;CLI 本次**不**做自动迁移。

### 3.5 不动 memory / 历史文件

- `.peaks/memory/` 110 个文件:全部不动。理由:memory 是事实快照,2026-07-04 之前的 memory 提到 peaks-solo 是历史事实;2026-07-04 之后的新 memory 自然会用 peaks-code。一致性由"日期"维度保证,不是"命名"维度。
...
`/peaks-solo` → `/peaks-code`(用户从 IDE 调起的 slash command)
所有 SKILL.md 里的 trigger 字符串同步;测试里 hard-coded 的 `/peaks-solo` 字符串全替换。
...
| `tests/fixtures/skills/pre-slim/peaks-solo.SKILL.md` | 1 文件 | 改名 + 内部 frontmatter |
...
| `scripts/`(若有) | 多处 | 改 |
| `skills/peaks-solo*/SKILL.md` | 4 个文件 | 改 frontmatter |
| `skills/peaks-solo*/references/*` | 多文件 | 改 self-reference |
| `skills/peaks-solo*/test-prompts.json` | 1 文件 | 改 |
...
```bash
# 先 release-build,把改完的 skill 写入 .peaks/skills/.system/bees/peaks-solo/manifest.json(id 已改 peaks-code)
...
- [ ] **5.1.1** 创建 branch:`git checkout -b feature/peaks-solo-to-peaks-code`(基于当前 `feature/4x-sediment-pool`)
- [ ] **5.1.2** 创建 changelog 草稿条目:`### Renamed`(在 CHANGELOG.md 顶部 `## [Unreleased]` 下)

### 5.2 Source 改动(commit 1,atomic)

- [ ] **5.2.1** `git mv skills/peaks-solo skills/peaks-code`
- [ ] **5.2.2** `git mv skills/peaks-solo-resume skills/peaks-resume`
- [ ] **5.2.3** `git mv skills/peaks-solo-status skills/peaks-status`
- [ ] **5.2.4** `git mv skills/peaks-solo-test skills/peaks-test`
...
- [ ] **5.3.2** 全量 grep-replace:`rg -l "peaks-solo" src/ tests/ scripts/ .claude/ .claude-plugin/ docs/`(排除 `.peaks/memory`、`openspec/`、`.git/sdd/`)
- [ ] **5.3.3** `tests/fixtures/skills/pre-slim/peaks-solo.SKILL.md` 改名 `peaks-code.SKILL.md`(若 fixture 名包含子技能名,一并改)
...
- [ ] **5.4.1** `.claude-plugin/marketplace.json`:`peaks-solo*` 条目 id 全改
- [ ] **5.4.2** `.claude/LOGGING.md`:全文替换
- [ ] **5.4.3** `docs/superpowers/specs/2026-07-04-peaks-maker-dynamic-skill-sediment-design.md` §4.1.1 段落改写:从"preserved as alias"改为"renamed in v4.1 to peaks-code; physical path `.system/bees/peaks-solo/` preserved for backward-compat"
...
设计:
```bash
peaks session migrate-skill-name --from peaks-solo --to peaks-code [--apply] [--project <repo>] [--json]
```
...
- **`--apply`** —— 实际写盘。改写范围:
  ...
  - `.peaks/_runtime/<sessionId>/<role>/*.json`(任意 role 产物 JSON)里出现的 `"skill": "peaks-solo"` 键值
  - `.peaks/_runtime/<sessionId>/txt/*.md`、`requests/*.md` 里的 `/peaks-solo` 触发字符串(仅在 frontmatter `trigger:` 字段或代码块里)
- **不改:**
  - `.peaks/memory/**`(memory 是事实快照,见 §2.1)
  - `.peaks/skills/.system/bees/peaks-solo/manifest.json`(manifest 的 `id` 字段改在 §5.4 doc 改动里手写改,不是这步)
...
- [ ] **5.5.2.2** `peaks session migrate-skill-name --from peaks-solo --to peaks-code --project .`(dry-run)→ 输出 `scannedFiles / modifiedFiles` 计数
- [ ] **5.5.2.3** 在 `.peaks/_runtime/` 下创建 1 个 fixture `tests/fixtures/runtime/session-with-peaks-solo.json`,故意带 `"skill": "peaks-solo"`,跑 `--apply` 后断言 fixture 内容已改
- [ ] **5.5.2.4** `peaks session migrate-skill-name --from peaks-solo --to peaks-code --apply --project .` → 真实跑一次,捕获 stdout
- [ ] **5.5.2.5** `rg "peaks-solo" .peaks/_runtime/` → 输出必须为空(或仅命中 §5.5.1 列出的"不改"白名单)
...
5. 跳过 .peaks/memory/**
6. 跳过 .peaks/skills/.system/bees/peaks-solo/manifest.json
...
- [ ] **5.6.4** 在 PR 描述里写 "Breaking change: skill id `peaks-solo` → `peaks-code`; sub-skill ids `peaks-solo-resume / -status / -test` → `peaks-resume / -status / -test`. Pool physical path preserved. Manual migration required for `.peaks/_runtime/**/*.json` `skill:` fields."
...
3. `peaks session migrate-skill-name --from peaks-code --to peaks-solo --apply --project .`(反向迁移,新 CLI 本身支持双向)
4. `peaks skill presence:set peaks-solo --gate startup`
...
| Skill presence 文件 user 忘记手动 sed | 高——下一次 LLM 启动读到 `skill: "peaks-solo"` 会困惑 | 5.5.1 是显式 user action;5.6.4 的 PR 描述里写 "Manual migration required" |
| 旧 slash command `/peaks-solo` 在 IDE 中仍然可触发,但触发到的 skill 已被 rename,IDE 报错 | 中 | 在 changelog 和 PR 描述里写明 "slash command `/peaks-solo` no longer valid; use `/peaks-code`";不回退旧 trigger(回退 = 半步更名,违反 user "一次到位"决策) |
| 旧 trigger 字符串 `/peaks-solo-resume` 等三个 sub-skill 的 slash command 也作废 | 中 | 同上,在 changelog 写明 |

---

## 8. 验收标准(Acceptance Criteria)

**AC-1:** `rg "peaks-solo" src/ tests/ scripts/ .claude/ .claude-plugin/ skills/ docs/superpowers/` 输出为空(扣除 spec 里"renamed in v4.1 to peaks-code"这类历史叙述)。
...
# → 全部存在
ls skills/peaks-solo skills/peaks-solo-resume skills/peaks-solo-status skills/peaks-solo-test
...
**AC-11:** `tests/unit/cli/session-migrate-skill-name.test.ts` 8 个 case 全绿,且 dogfood 5.5.2.5 的 `rg "peaks-solo" .peaks/_runtime/` 在排除白名单后输出为空。
...
- `.peaks/memory/peaks-solo-is-an-orchestrator-not-an-implementer-even-for-pure-doc-changes.md`(描述的主技能行为,改 id 后行为不变)