# peaks-loop L1+L2+L3 战略重设计 — Design Spec
...
| # | 痛点原话 | 技术翻译 |
|---|---|---|
| #1 | "LLM 没有深刻理解项目，缺性能优化视角、代码写法治理" | **项目级判断缺失**——peaks-loop 缺少独立于用户经验之外、能从项目客观信号派生判断的角色 |
| #2 | "LLM 也会偷懒能跳过就跳过，需要极客精神" | **LLM 行为漂移**——peaks-loop 的 MANDATORY / BLOCKING 红线大多 prose-only，LLM 可轻易绕过 |
| #3 | "改个简单 bug 要半小时，即使使用蜂群模式也很慢" | **gate 颗粒度与任务颗粒度不匹配**——所有任务（typo / bug / feature）走同一套 gate，开销 >> 实际工作量 |
| #4 | "context 怎么动态精简，但怕拖累效率和让 LLM 漂移" | **context 体积失控**——peaks-loop 当前 L2/L3 context 大半全量 load，没有按需检索机制 |

### 根因 (Root Cause)

> **peaks-loop 是"流程主义"——所有任务走一样的流程，所有 gate 一样的颗粒度，所有圈定一样的来源 (用户经验)。**
...
这不是个人困惑，是 AI Coding 工具链的**根本性悖论**。peaks-loop 当前**完全偏向"圈定"那一边**（skill-first / red-line gates / memory / retrospective / dev-preference.md 全部是用人的视角圈定 LLM），代价是：**它原样继承了用户的盲区**。
...
│  └── peaks-loop 自有红线 CLI 兜底 (~120-150 条 prose-only red lines)│
│  解 #2 LLM 偷懒                                                   │
└────────────────────────────────────────────────────────────────┘
                          │
                          ▼ 让 L3 诊断不被 LLM 跳过
...
│  ├── 输入: UA knowledge-graph + ECC agents + peaks-loop 内置诊断器  │
│  ├── 处理: severity 判定 + 路由分发                                │
...
**peaks-loop 不是 Claude-Code 专属工具。** 本设计在 L1/L2/L3 三层之上**显式声明**多 AI agent CLI 支持要求, 复用 peaks-loop 1.3.3+ 引入的 IDE 适配层 (`src/services/ide/`)。
...
新增 adapter 工作量**极小** (per `ide-types.ts` 注释: "新 IDE 适配变成'填表'"), 因为 peaks-loop 已经把 IDE 私有 hook 协议归一化, 每个 adapter 只需要填:
...
| **Skill 调用机制** | ⚠️ 平台相关 | 每平台有不同的 skill / slash command 格式; peaks-loop 提供 CLI 后端, skill 前端各平台独立 |
...
**peaks-loop 的产品是 CLI + Skills 双轨**——CLI 是命令行原语, Skills 是编排 prompt。L1+L2+L3 重设计不仅改 CLI, 还必须同步整改 skills, 否则 CLI 改了但 skill 仍以旧 model 工作, **架构整体不一致**。

#### 现状审计 (`c:/Users/smallMark/Desktop/peaks-loop/skills/`)
...
6. **Output Style Discipline** (统一 skill 回答风格, 让 LLM 输出简洁)
   - 现状: CLAUDE.md 隐含 `Peaks-Loop Skill: <skill> | Peaks-Loop Gate: <gate> | Next: <action>` header, 但散在各 skill 中, 强度不一
...
| 框架 | 用 peaks-loop 角度评估 |
|---|---|
| [OpenAI Swarm](https://github.com/openai/swarm) | ❌ Python only + 被 Agents SDK 取代 |
| [LangGraph](https://github.com/langchain-ai/langgraph) | ⚠️ 可借鉴 DAG 算法, 不宜整体引入 (TS 版有, 但跟 peaks-loop 的"orchestrator of orchestration"角色重复) |
...
**关键诊断**: peaks-loop 是 **orchestrator of orchestration** (生成 toolCall descriptor 让平台 AI agent runtime 执行), 直接引入运行时 agent 框架会跟平台 agent runtime 重复, 还要绕 IPC。

**正确路径**: **借用算法模式, 不引入运行时依赖**。

#### 推荐: 5 个开源经典模式 (零依赖, 借用 not import)

| 模式 | 来源 | 解什么 | peaks-loop 落点 |
|---|---|---|---|
...
**Claude Code 一等公民 `/goal` (v2.1.139+, [官方文档](https://code.claude.com/docs/zh-CN/goal))** 提供了干净的 condition-driven turn chaining 原语。peaks-loop L4 直接借力, **不重复发明**:

| `/goal` 特性 | peaks-loop L4 14.5 利用方式 |
|---|---|
...
peaks audit static (L2 静态分析入口)
   ...
   └── 未装 → 询问用户是否安装 (复用 UA 的"用户选装"UX, 见 §7.2):
              ...
              ├── b) 这次不装 (用 peaks-loop 退化版简单 lint)
              ├── c) 永远不装
              └── d) 先了解一下
```
...
2. **选择 enforcement 机制**：
   - 静态规则 → AgentShield rule 或 peaks-loop ESLint rule
...
**集成模式**: peaks-loop **不调用** UA pipeline, 只**读** `.understand-anything/knowledge-graph.json`。
...
检测 .understand-anything/knowledge-graph.json 是否存在?
   │
   ├── 存在 → ✅ 使用 UA graph + ECC agents + peaks-loop 内置诊断 → JSON 输出 → 路由
   │
...
| 降级保底 | UA 未装时 peaks-loop 用 `@colbymchenry/codegraph` (已在 deps) 跑退化版 L3 |
...
| ECC 维度 | 集成姿态 | 理由 |
|---|---|---|
| **rules/common + rules/<lang>** | **Hard required** (维持现状) | peaks-loop `.claude/rules/` 已基于 ECC baseline, 持续 sync |
| **AgentShield** (102 静态规则 + 1282 测试) | **Soft Optional** | 装了 L2 直接用; 不装退化到简单 lint。复用 UA 同款"用户选装"UX |
| **64 agents** (诊断器) | **Soft Optional** | 装了 L3 直接调; 不装 L3 退化到 peaks-loop 自有少数核心诊断 |
...
- peaks-loop 自动 fallback 到不压缩, 继续工作 — **永远不阻塞**
...
#### `preferences.json` schema (项目级, peaks-loop 2.0)
...
- [ ] dogfood: 在 peaks-loop 本仓库跑全套迁移, `~/.peaks/config.json` 缩到 1 个 key, `.peaks/preferences.json` 正确写入, 旧 session 归档到 `_archive/2026-06/`
...
- [ ] `peaks skills sync --ide all --dry-run` 8 平台分发预演无错误
- [ ] dogfood: 在 peaks-loop 仓库执行 `peaks skills sync --ide claude-code --apply`, 验证 `~/.claude/skills/peaks-*/SKILL.md` 与仓库 skills/ source 字节一致
...
- [ ] dogfood: 在 peaks-loop 仓库跑一次 feature gate, 测量主循环 input token 总量降低 ≥ 15% (基准: 关 headroom 跑一次, 开 headroom 跑一次, 对比)
...
- [ ] dogfood: 在 peaks-loop 仓库跑 5 个连续 slice, 验证 (a) memory/lessons/auto-*.md 累积 (b) 5 slice 后 preflight 命中率从 0 升到 ≥ 60% (c) 跨 slice 决策一致性无矛盾
...
2. **ECC AgentShield 102 条规则跟 peaks-loop 自有 lint 规则的优先级冲突时, 谁覆盖谁?**
...
- 不做付费版 / 商业化 (peaks-loop 保持开源)
- L3 项目医生 MVP 阶段只做 1-2 个 peaks-loop 内置诊断器 (剩余靠 ECC agents 编排), 不做 ECC 之外的全套诊断
- **`~/.peaks/credentials.json` / `providers.json` / `network.json` / `workspaces/`** 是前瞻设计, **本设计 Slice 0.5 不实现**, 等真需求出现时再独立 slice 迭代加
- **不绑定任何单一 AI agent CLI** (peaks-loop 是 multi-platform orchestration 工具, 不是 Claude-Code-only)。所有 L1/L2/L3 核心组件必须通过 IDE 适配层 (`src/services/ide/`) 在 8 个目标平台上工作: claude-code / codex / trae / cursor / qoder / tongyi-lingma / hermes / openclaw。
...
- **不引入运行时 agent 框架依赖** (LangGraph / CrewAI / AutoGen / OpenAI Swarm 都不 import — peaks-loop 是 orchestrator of orchestration, 不跟平台 agent runtime 重叠。Slice 13 借用算法模式, 不引入运行时依赖)
- **不实现 BYOA (Bring Your Own Agent runtime)** — 用户用什么平台 (Claude Code / Codex / Cursor / ...) 就走平台的 agent runtime, peaks-loop 只生成 toolCall descriptor
