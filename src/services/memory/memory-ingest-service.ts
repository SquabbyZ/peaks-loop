...
import {
  ...
  writeNewFile
} from './project-memory-service/store/atomic-write.js';
...
function emptyReport(
  overrides: Partial<MemoryIngestReport> &
    Pick<MemoryIngestReport, 'projectRoot' | 'sourceDir' | 'memoryDir'>
): MemoryIngestReport {
  return {
    apply: false,
    sourceExists: false,
...
export function executeMemoryIngest(options: MemoryIngestOptions): MemoryIngestReport {
  ...
  if (indexFiles.length > 0) {
    report.warnings.push(
      `Skipped ${indexFiles.length} IDE-side index file(s) named ${IDE_INDEX_FILENAME} (generated by the IDE, not a memory).`
    );
  }
  ...
  for (const sourcePath of memoryFiles) {
    ...
    const normalized = renderNormalizedMemory({
      stem,
      kind,
    ...
    if (existsSync(targetPath)) {
      ...
      try {
        ...
        report.conflicts.push({
          ...
          sourcePath,
          targetPath,
          reason: 'destination exists but could not be read'
        });
        continue;
      }
      if (existing === normalized) {
        ...
        report.conflicts.push({
          ...
          sourcePath,
          targetPath,
          reason: 'destination exists with different content; both copies left untouched'
        });
      }
      continue;
    }
    ...
    if (stableMemoryDir === null || !isInsidePath(stableTargetPath, stableMemoryDir)) {
      report.refused.push({
        name: stem,
        sourcePath,
        reason: 'refused: target path escapes the project memory directory'
      });
      continue;
    }
