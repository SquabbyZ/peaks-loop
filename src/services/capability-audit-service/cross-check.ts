...
export function crossCheck(input: {
  readonly guardPass: number;
  readonly guardFail: number;
  readonly independentPass: number;
  readonly independentFail: number;
  ...
  const guardVsAudit =
    guardVerdict === indepVerdict
      ? 'agree'
      : Math.abs(input.guardPass - input.independentPass) <= 1
        ? 'partial'
        : 'diverge';
  const karpathyVsAudit =
    input.karpathy === 'skipped'
      ? 'partial'
      : input.karpathy === 'warn'
        ? 'partial'
        : input.karpathy === 'pass'
