 ...
 *
 * M1 scope (this file): the §4.1 fields EXCEPT the M3 share / desktop
 * extension fields (shareable, share_excluded_paths, desktop_visible,
 * export_bundle_format). M3 adds those as optional fields; adding them
 * later does not change the §4.1 contract.
 *
...
/**
 * The M3 share / desktop extension fields, intentionally excluded
 * from M1. Listed here so future readers see exactly what is NOT yet
 * on the schema. Adding these in M3 must remain additive (optional,
 * defaulting to their spec values) — no breaking change to existing
 * M1 rows.
 *
 *   - shareable: defaults to true.
 *   - share_excluded_paths: string[] (optional).
 *   - desktop_visible: defaults to true.
 *   - export_bundle_format: constant "peaks.bundle/1" (CLI-written).
 */
export const LOOP_RELEASE_M3_RESERVED_FIELDS = [
  "shareable",
 ...
 */
export const LoopReleaseInputSchema = z.object({
  id: z
    .string()
    .min(1)
    .max(64)
