...
function readRegistrations(input: RunMergeBackInput): ReadonlyArray<ServiceRegistration> {
  ...
  try {
    return JSON.parse(readFileSync(f, 'utf8')) as ReadonlyArray<ServiceRegistration>;
  } catch {
    return [];
  }
}
...
function captureConflictDiff(input: RunMergeBackInput): string {
  try {
    return execFileSync('git', ['diff', '--merge', '--no-color'], {
...
function captureTranscript(input: RunMergeBackInput): ReadonlyArray<string> {
  try {
    return execFileSync('git', ['merge', '--no-edit', '--no-ff', input.agentBranch], {
      cwd: input.projectRoot,
      encoding: 'utf8',
      windowsHide: true
    }).split('\n');
  } catch {
    return ['git merge --no-edit --no-ff ' + input.agentBranch];
  }
}
...
export async function runMergeBack(input: RunMergeBackInput): Promise<RunMergeBackResult> {
  ...
  while (attempts < 2) {
    ...
    if (plan.kind === 'conflict') {
      ...
      const replay = buildConflictReplay({
        originalPrompt,
      ...
      if (!replayResult.ok) {
        return {
          kind: attempts >= 2 ? 'replay-exhausted' : 'replay-still-conflict',
          attempts,
          serviceKills: kills
        };
      }
    ...
    try {
      execFileSync('git', ['checkout', input.callerBranch], {
        cwd: input.projectRoot,
        stdio: 'ignore',
        windowsHide: true
      });
      execFileSync(plan.command[0] as string, plan.command.slice(1), {
        cwd: input.projectRoot,
        stdio: 'ignore',
        windowsHide: true
      });
      ...
      const replay = buildConflictReplay({
        originalPrompt,
        mergeAttemptTranscript: transcript,
        conflictDiff,
        callerBranch: input.callerBranch
      });
      const replayResult = await input.onConflict(replay);
