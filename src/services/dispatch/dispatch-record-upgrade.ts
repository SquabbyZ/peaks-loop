...
function parseUpgradeRecordLegacyFields(obj: Record<string, unknown>): {
  ...
  const status: DispatchRecordStatus = isDispatchStatus(obj.status) ? obj.status : 'unreadable';
...
function parseUpgradeRecordMigrationFields(obj: Record<string, unknown>): {
  ...
  return {
    ...
    vendor:
      obj.vendor === 'claude' || obj.vendor === 'codex' || obj.vendor === 'copilot'
        ? obj.vendor
        : null,
