...
// `ensureSession` and `ensureSessionWithRotation` (plus the
// `EnsureSessionOptions` / `EnsureSessionResult` types) moved to
// `./session-binding-bridge.ts` in v2.18.0 to keep this file under
// the Karpathy 800 LOC cap. The re-export below preserves the public
// import path for the 5 external callers — see the bridge file header
// for the rationale. Do NOT add new logic here.
export {
  ensureSession,
  ensureSessionWithRotation,
  type EnsureSessionOptions,
  type EnsureSessionResult
} from './session-binding-bridge.js';
