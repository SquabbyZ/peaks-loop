...
export function evaluateCompactTrigger(ratio: number): CompactTrigger {
  ...
  return {
    ...
    ratio,
    toolkitReady: true,
    message: `Context at ${(ratio * 100).toFixed(1)}% in pre-compact zone (0.85–0.95). Convergence toolkit is ready; LLM decides when to fire \`peaks solo auto-compact --execute\`.`
  };
}
