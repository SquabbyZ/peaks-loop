...
const REPAIR_NOTE_PATTERN =
  /-\s*transition note\s*\(([^)]+)\)\s*:\s*(?:QA(?:\s+return-to-rd)?\s+cycle\s+(\d+))\s*:?\s*(.*?)$/i;

export async function getRepairCycleStatus(
  options: RepairCycleStatusOptions
): Promise<RepairCycleReport | null> {
