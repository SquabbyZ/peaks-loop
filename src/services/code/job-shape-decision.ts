 ...
 * `peaks code detect-job`; this service persists the verdict to
...
export function readJobShapeDecision(projectRoot: string, sessionId: string): JobShapeRecord {
  ...
  if (!existsSync(path)) {
    throw new JobShapeDecisionError({
      ...
      message: `No Job-shape decision recorded for session ${sessionId}. Run \`peaks code detect-job\` to record a decision.`,
      details: { path }
    });
  }
  let raw: string;
  try {
    ...
    throw new JobShapeDecisionError({
      code: JOB_SHAPE_NOT_DECIDED,
      message: `Job-shape decision file is unreadable for session ${sessionId}. Run \`peaks code detect-job\` to re-record.`,
  ...
  try {
    ...
    throw new JobShapeDecisionError({
      code: JOB_SHAPE_NOT_DECIDED,
      message: `Job-shape decision file is malformed JSON for session ${sessionId}. Run \`peaks code detect-job --force\` to overwrite.`,
      details: { path, cause: err instanceof Error ? err.message : String(err) }
    });
  }
  try {
    ...
    throw new JobShapeDecisionError({
      code: JOB_SHAPE_NOT_DECIDED,
      message: `Job-shape decision file is schema-invalid for session ${sessionId}. Run \`peaks code detect-job --force\` to overwrite.`,
      details: { path, cause: err instanceof Error ? err.message : String(err) }
    });
  }
}
