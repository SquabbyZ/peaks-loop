...
export const DEFAULT_REVIEW_ITEMS: readonly { id: string; question: string }[] = [
  ...
  {
    id: 'boundary-cases',
    question: '边界 case 跟业务预期一致吗?(异常输入/错误提示/空/加载/失败状态)'
  },
  {
    id: 'ui-assembly',
    question: 'UI 装配跟产品预期一致吗?(页面模式/关键交互/信息密度,非视觉设计)'
  },
  { id: 'mergeable', question: '能合入下个业务版本吗?(业务风险/集成断点)' }
];

export function buildEmptySliceReview(
  sliceId: string,
  sessionId: string,
  now: Date = new Date()
): SliceReview {
...
export function readSliceReview(
  projectRoot: string,
  sessionId: string,
  sliceId: string
): SliceReview | null {
