 ...
 */
async function probePrepareFinalReview(): Promise<{ readonly rejected: boolean; readonly accepted: boolean; readonly detail: string }> {
  ...
  try {
    ...
    let rejectName = 'none';
    try {
      await prepareFinalReview('J05-fixture', { ...opts, llmRunner: runnerReturning(reviewJson(FOUR_DIMENSIONS.slice(0, 3))) });
    } catch (e) {
    ...
    let acceptError = '';
    try {
      const out = await prepareFinalReview('J05-fixture', { ...opts, llmRunner: runnerReturning(reviewJson(FOUR_DIMENSIONS)) });
...
export async function runJ05Contract(ctx: GuardContext): Promise<GuardRunResult> {
  ...
  const oversized = FOUR_DIMENSIONS.map((d) => undeliverableSource(d, MAX_EVIDENCE_BYTES_PER_FILE + 1000));
  const reported = undeliverableDimensions(oversized as UndeliverableInput).map((r) => r.dimension);
  const fifthOnly = undeliverableDimensions([undeliverableSource(FIFTH_DIMENSION, MAX_EVIDENCE_BYTES_PER_FILE + 1000)] as UndeliverableInput);
  ...
  const result = combineProbes([
    ...
    probe(fifthOnly.length === 0, `the fifth dimension is not part of the required set (reported: ${fifthOnly.length})`),
    probe(MAX_EVIDENCE_BYTES_PER_FILE === 10240, `per-file evidence cap divides the total by four (${String(MAX_EVIDENCE_BYTES_PER_FILE)})`),
    probe(floor === 15288 && ceiling === 32000, `output budget is clamped on both ends (${String(floor)} / ${String(ceiling)})`),
    probe(badOverrideThrows, 'a non-integer budget override throws instead of falling back silently')
  ]);
