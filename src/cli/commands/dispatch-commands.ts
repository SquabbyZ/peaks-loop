...
export function registerDispatchCommand(parent: Command, io: ProgramIO): void {
  addJsonOption(
    parent
      ...
      .option('--emit-bash-script', 'Slice 4.0.7-PR-meta-4: in addition to the JSON envelope, emit a ready-to-exec bash script that the LLM (or the user) can run via the Bash tool. The script invokes the Task tool\'s underlying sub-agent runtime. Lowers the dispatch friction documented in `.peaks/memory/2026-07-28-sub-agent-visibility-issue.md` (the LLM no longer has to manually extract toolCall args from the envelope). The dry-run architecture is preserved: the script is a one-shot executor, not a side-step.')
    ...
    try {
      ...
      const isolationBlock = isolationMode !== null && leaseId !== null
        ...
        : '';
      // Slice 4.0.7-dogfood-PR-9: when pnpm-workspace.yaml is present,
      // append the monorepo test-tool block so the sub-agent scopes
      // to a single sub-package instead of fanning out to the whole
      // monorepo. The block is empty/cheap when the project is a
      // single package (no pnpm-workspace.yaml).
      const isMonorepo = existsSync(join(projectRoot, 'pnpm-workspace.yaml'));
      const monorepoBlock = isMonorepo ? `\n\n${MONOREPO_TEST_TOOL_BLOCK}` : '';
      let effectivePrompt = `${formatTestToolDetection()}${monorepoBlock}\n\n${memoryAugmentedBody}${isolationBlock}`;
      ...
      // Slice 4.0.7-PR-meta-4: --emit-bash-script prints a one-shot
      // bash script to stdout that the LLM (or the user) can run
      // directly via the Bash tool. The script invokes the
      // canonical `peaks sub-agent exec` surface (a new CLI in
      // this slice) that re-runs the dispatch with the same
      // dispatchRecordPath, sid, role, batch-id, rid, and
      // prompt-size, so the LLM does not have to parse toolCall
      // args from the JSON envelope and re-execute them. The
      // dry-run architecture is preserved: the CLI is the
      // canonical dispatch surface; the script is a one-shot
      // executor, not a side-step.
      if (options.emitBashScript === true) {
        const script = [
          '#!/usr/bin/env bash',
          `# Slice 4.0.7-PR-meta-4: dispatch executor for rid=${rid} role=${role} batch=${batchId}`,
          `# Generated by \`peaks sub-agent dispatch --emit-bash-script\`. Re-invokes the`,
          '# canonical peaks sub-agent exec surface; safe to run via Bash tool.',
          `peaks sub-agent exec \\`,
          `  --session-id "${sid}" \\`,
          `  --request-id "${rid}" \\`,
          `  --role "${role}" \\`,
          `  --batch-id "${batchId}" \\`,
          `  --dispatch-record "${dispatchRecordPath}" \\`,
          `  --project "${projectRoot}"`
        ].join('\n');
        process.stdout.write(`\n# === peaks sub-agent dispatch — ready-to-exec script (slice 4.0.7-PR-meta-4) ===\n${script}\n# === end script ===\n`);
      }
