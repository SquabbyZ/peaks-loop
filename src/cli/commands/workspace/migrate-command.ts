...
export function registerWorkspaceMigrateCommand(workspace: Command, io: ProgramIO): void {
  addJsonOption(
    workspace
      ...
      .option(
        '--apply',
        'actually `git mv` the files and delete the emptied session dirs (destructive); without it, dry-run only',
        false
      )
      .option(
        '--to-runtime',
    ...
    try {
      ...
      if (result.sessions.length === 0 && (result.toRuntimePlans?.length ?? 0) === 0) {
        ...
        warnings.push(
          'Legacy session dirs found but no reviewable content to migrate (all files were cross-cutting or transient).'
        );
      }
      ...
      if (!apply && result.wouldMove.length > 0) {
        nextActions.push(
          `Re-run with --apply to perform ${result.wouldMove.length} move(s) and delete ${result.wouldDeleteSessions.length} session dir(s).`
        );
      }
      if (result.conflicts.length > 0) {
        nextActions.push(
          `${result.conflicts.length} file(s) already exist at the target path; review before --apply (or re-run after a partial migrate).`
        );
      }
      if (toRuntime) {
        ...
        if (apply) {
          if ((result.toRuntimeMoved?.length ?? 0) > 0) {
            nextActions.push(
              `Moved ${result.toRuntimeMoved?.length} top-level session dir(s) to .peaks/_runtime/ (slice 003 --to-runtime).`
            );
          }
          if ((result.toRuntimeConflicts?.length ?? 0) > 0) {
            nextActions.push(
              `${result.toRuntimeConflicts?.length} --to-runtime conflict(s) — see response. ${plans.filter((p) => p.action === 'f15-conflict-project-scan').length} are F15 carve-outs (deferred to a separate slice).`
            );
          }
        } else {
          const wouldMoveCount = plans.filter((p) => p.action === 'moved').length;
          const wouldSkipCount = plans.filter(
            (p) => p.action === 'skipped-already-canonical'
          ).length;
          if (wouldMoveCount > 0) {
            nextActions.push(
              `Re-run with --apply to move ${wouldMoveCount} top-level session dir(s) to .peaks/_runtime/; ${wouldSkipCount} already canonical.`
            );
          } else if (wouldSkipCount > 0) {
            nextActions.push(
              `All ${wouldSkipCount} top-level session dir(s) are already canonical — no moves needed.`
            );
          }
          const f15Count = plans.filter((p) => p.action === 'f15-conflict-project-scan').length;
          if (f15Count > 0) {
            nextActions.push(
              `${f15Count} F15 carve-out conflict(s) (rd/project-scan.md differs from runtime copy) — see response.`
            );
          }
        }
      }
      ...
      printResult(
        io,
        ok('workspace.migrate', result, warnings, nextActions),
        options.json ?? false
      );
    } catch (error) {
      printResult(
