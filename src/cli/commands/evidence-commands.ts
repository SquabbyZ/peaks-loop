...
import {
  ...
  parseFiles,
  parseLineCounts
} from '../../services/evidence/evidence-generator.js';
...
export function registerEvidenceCommands(program: Command, io: ProgramIO): void {
  const evidence = program
    .command('evidence')
    .description('Generate per-slice evidence artifacts for mechanical slices');

  addJsonOption(
    evidence
      .command('generate')
      .description(
        'Write the ~11 rd/qa/audit/prd evidence artifacts for a mechanical file-split slice under .peaks/_runtime/<sid>/'
      )
      ...
      .requiredOption(
        '--line-counts <list>',
        'semicolon-separated file=lines pairs (e.g. "f=120;g=300")'
      )
    ...
    try {
      ...
      printResult(
        io,
        fail('evidence.generate', 'EVIDENCE_GENERATE_FAILED', getErrorMessage(error), {}, [
          'Verify the target project is writable.'
        ]),
        options.json
      );
      process.exitCode = 1;
