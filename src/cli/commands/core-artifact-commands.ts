...
export function registerCoreAndArtifactCommands(program: Command, io: ProgramIO): void {
  ...
  addJsonOption(program.command('doctor').description('Run repository doctor checks').option('--log', 'include a "logs" section in the doctor output (slice 2026-06-16-cli-logging, AC6)')).action(async (options: { json?: boolean; log?: boolean }) => {
