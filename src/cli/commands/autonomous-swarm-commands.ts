...
function projectContext(): {
  ...
  workspace?: PlanAutonomousRdSwarmInput['workspace' & never];
} {
...
export function runAutonomousSwarmPlan(io: ProgramIO, options: AutonomousPlanOptions): void {
  if (options.dryRun === false) {
    printResult(
      ...
      fail(
        'workflow.autonomous',
        'NON_DRY_RUN_UNSUPPORTED',
        'Only dry-run planning is supported',
        {},
  ...
  if (!isAutonomousMode(options.mode)) {
    printResult(
      ...
      fail(
        'workflow.autonomous',
        'UNSUPPORTED_AUTONOMOUS_MODE',
        `Unsupported autonomous mode ${options.mode}`,
        {},
  ...
  try {
    ...
    if (error instanceof Error && /changeId|change-id/.test(error.message)) {
      if (error.message.includes('empty')) {
        printResult(
          io,
          fail('workflow.autonomous', 'change-id-empty', error.message, {}, [
            'Provide a change-id matching [A-Za-z0-9][A-Za-z0-9._-]*'
          ]),
          options.json
        );
      } else {
        printResult(
          io,
          fail('workflow.autonomous', 'change-id-format', error.message, {}, [
            'Provide a change-id matching [A-Za-z0-9][A-Za-z0-9._-]*'
          ]),
          options.json
        );
...
export function registerAutonomousSwarmCommands(program: Command, io: ProgramIO): void {
  ...
  if (workflow) {
    addAutonomousPlanOptions(workflow.command('autonomous-swarm')).action(
      (options: AutonomousPlanOptions) => runAutonomousSwarmPlan(io, options)
    );
  }
  addAutonomousPlanOptions(program.command('autonomous-swarm')).action(
    (options: AutonomousPlanOptions) => runAutonomousSwarmPlan(io, options)
  );
}
