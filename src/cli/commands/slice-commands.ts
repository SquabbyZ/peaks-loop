...
import {
  readResult as readDecompositionResult,
  writeResult as writeSchemaResult
} from '../../services/slice/schema-router.js';
...
export function registerSliceCommands(program: Command, io: ProgramIO): void {
  ...
  addJsonOption(
    ...
    async (options: {
      ...
      try {
        ...
        printResult(
          io,
          ok('slice.check', result, warnings, result.nextActions),
          options.json ?? false
        );
      ...
      try {
        ...
        if (options.json === true) {
          ...
          if (page.some((r) => r.isStale)) {
            nextActions.push(
              'Some entries are stale (>30d). Use `peaks slice cleanup` to remove them.'
            );
          }
          printResult(
            io,
  ...
  addJsonOption(
    ...
    async (
      rid: string,
      options: { project: string; preview?: boolean; fzfBin?: string; json?: boolean }
    ) => {
      try {
        const projectRoot = resolveCanonicalProjectRoot(options.project);
  ...
  addJsonOption(
    ...
    try {
      ...
      printResult(
        io,
        ok('slice.plan', { parentRid: rid, plan, apply: options.apply ?? false }, [], nextActions),
        options.json ?? false
      );
    } catch (error) {
      const msg = getErrorMessage(error);
...
function writeDecompositionFile(
  rid: string,
