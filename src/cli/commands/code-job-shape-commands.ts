...
export function registerCodeJobShapeCommands(code: Command, io: ProgramIO): void {
  ...
  addJsonOption(
    code
      ...
      .requiredOption(
        '--suggested-job-id <jid>',
        'LLM-chosen stable id, slug-safe (/^[a-z0-9][a-z0-9-]{2,40}$/)'
      )
      ...
      .option(
        '--prompt <text>',
        'override the prompt hashed into promptHash (default: read last-prompt.txt if present, else empty)'
      )
  ).action(
    async (opts: {
      isJob: string;
      ...
      try {
        if (opts.isJob !== 'true' && opts.isJob !== 'false') {
          printResult(
            ...
            fail(
              'code.detect-job',
              'INVALID_FLAG',
              `--is-job must be true | false (got "${opts.isJob}")`,
              { provided: opts.isJob },
              ['Pass --is-job true or --is-job false']
            ),
            opts.json
          );
          process.exitCode = 1;
        ...
        if (opts.suggestedStrategy !== 'single' && opts.suggestedStrategy !== 'rotating') {
          printResult(
            ...
            fail(
              'code.detect-job',
              'INVALID_FLAG',
              `--suggested-strategy must be single | rotating (got "${opts.suggestedStrategy}")`,
              { provided: opts.suggestedStrategy },
              ['Pass --suggested-strategy single or --suggested-strategy rotating']
            ),
            opts.json
          );
          process.exitCode = 1;
        ...
        if (
          ...
          printResult(
            ...
            fail(
              'code.detect-job',
              'INVALID_FLAG',
              `--confidence must be high | medium | low (got "${opts.confidence}")`,
              { provided: opts.confidence },
        ...
        if (!jidRe.test(opts.suggestedJobId)) {
          printResult(
            ...
            fail(
              'code.detect-job',
              'INVALID_FLAG',
              `--suggested-job-id must match /^[a-z0-9][a-z0-9-]{2,40}$/ (got "${opts.suggestedJobId}")`,
              { provided: opts.suggestedJobId },
              ['Pass a slug-safe id (lowercase, digits, dashes; 3-41 chars)']
            ),
            opts.json
          );
          process.exitCode = 1;
        ...
        if (isUnsafePathInput(sessionId)) {
          printResult(
            ...
            fail(
              'code.detect-job',
              'INVALID_SESSION_ID',
              `Invalid session id: ${sessionId} (must be a single path segment)`,
              { provided: sessionId },
        ...
        if (opts.prompt === undefined) {
          ...
          if (existsSync(lastPromptPath)) {
            try {
              ...
              // TODO(g2): legacy silent catch — grace: 1 minor release
              promptText = '';
            }
          }
        }
        ...
        printResult(
          io,
          fail('code.detect-job', 'DETECT_JOB_FAILED', getErrorMessage(err), null, [
            'Verify the flags and try again'
          ]),
          opts.json
        );
        process.exitCode = 1;
