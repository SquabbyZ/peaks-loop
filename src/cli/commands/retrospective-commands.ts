...
export function runRetrospectiveSearch(
  ...
  const typeFilter =
    options.type !== undefined && VALID_RETRO_TYPES.includes(options.type as RetrospectiveType)
      ? (options.type as RetrospectiveType)
      : undefined;
  const outcomeFilter =
    options.outcome !== undefined &&
    VALID_RETRO_OUTCOMES.includes(options.outcome as RetrospectiveOutcome)
  ...
  try {
    ...
    printResult(
      ...
      fail(
        ...
        code,
        message,
        { projectRoot, query: options.query },
        suggestions
      ),
      options.json
    );
...
export function registerRetrospectiveCommands(program: Command, io: ProgramIO): void {
  ...
  addJsonOption(
    ...
    async (options: { pick?: boolean; fzfBin?: string; project?: string; json?: boolean }) => {
      const projectRoot =
        options.project !== undefined
          ? resolveCanonicalProjectRoot(options.project)
          : (findProjectRoot(process.cwd()) ?? process.cwd());
    ...
    try {
      ...
      if (!result.ok) {
        ...
        if (result.code === 'ARTIFACT_MISSING' || result.missingArtifacts !== undefined) {
          suggestions.push(
            'Restore the missing artifact at the path listed under missingArtifacts'
          );
        }
        printResult(
          io,
          fail(
            ...
            result.message,
            {
              id,
              projectRoot,
              ...(result.missingArtifacts !== undefined
