...
function registerSCStatusCommands(sc: Command, io: ProgramIO): void {
  addJsonOption(sc.command('status').description('Show change traceability status')).action(
    (options: { json?: boolean }) => {
      printResult(io, ok('sc.status', getChangeTraceabilityStatus()), options.json);
    }
  );

  addJsonOption(sc.command('help').description('Show peaks-sc help text')).action(
    (options: { json?: boolean }) => {
...
function registerSCArtifactCommands(sc: Command, io: ProgramIO): void {
  ...
  ).action((options: { module?: string[]; file?: string[]; json?: boolean }) => {
    const impactOptions: {
      sessionId: string;
      ...
      printResult(
        ...
        ok(
          ...
          createArtifactRetentionReport({
            ...
            ...(options.prd ? { prdArtifacts: options.prd } : {}),
            ...(options.rd ? { rdArtifacts: options.rd } : {}),
            ...(options.qa ? { qaArtifacts: options.qa } : {}),
            ...(options.coverage ? { coverageArtifacts: options.coverage } : {}),
            ...(options.review ? { reviewArtifacts: options.review } : {}),
            ...(options.code ? { codeChanges: options.code } : {})
          })
        ),
        options.json
      );
  ...
  addJsonOption(
    sc
      .command('validate')
      .description('Validate artifact retention for a slice')
      .requiredOption('--slice-id <id>', 'slice identifier')
  ).action((options: { sliceId: string; json?: boolean }) => {
  ...
  addJsonOption(
    sc
      .command('boundary')
      .description('Record retention boundary for a slice')
      .requiredOption('--slice-id <id>', 'slice identifier')
    ...
    printResult(
      ...
      ok(
        ...
        recordCommitBoundary({
          ...
          ...(options.code ? { codeFiles: options.code } : {})
        })
      ),
      options.json
    );
