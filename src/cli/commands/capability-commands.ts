...
} from '../../services/recommendations/seed-capability-catalog.js';
...
const CAPABILITY_SOURCE_FILTERS = new Set<CapabilityMapSourceFilter>([
  'all',
  'access-repo',
  'mcp-server'
]);
...
export function registerCapabilityCommands(program: Command, io: ProgramIO): void {
  const capability = program
    .command('capability')
    .description('Inspect Peaks capability catalog and runtime availability');
  addJsonOption(
    capability.command('status').description('Show seed capability availability')
  ).action((options: { json?: boolean }) => runCapabilityStatus(io, options));

  addCapabilityMapOptions(
    capability.command('map').description('Show dry-run external capability landing map')
  ).action((options: CapabilityMapOptions) => runCapabilityMap(io, options));
  addCapabilityMapOptions(
    program.command('capabilities').description('Show dry-run external capability landing map')
  ).action((options: CapabilityMapOptions) => runCapabilityMap(io, options));
}
...
export function runCapabilityStatus(io: ProgramIO, options: { json?: boolean }): void {
  ...
  printResult(
    io,
    ok('capability.status', {
      sources: seedCapabilitySources,
      items: seedCapabilityItems,
      availability
    }),
    options.json
  );
...
function addCapabilityMapOptions(command: Command): Command {
  return addJsonOption(
    command.option(
      '--source <source>',
      'Filter source group: all, access-repo, or mcp-server',
      'all'
    )
  );
}
  ...
  if (!source) {
    printResult(
      ...
      fail(
        ...
        'Supported capability sources are all, access-repo, and mcp-server',
        { source: options.source },
        ['Rerun with --source all, --source access-repo, or --source mcp-server']
      ),
      options.json
    );
    process.exitCode = 1;
