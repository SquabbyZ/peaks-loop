...
export function registerScanCommands(program: Command, io: ProgramIO): void {
  ...
  addJsonOption(
    ...
    async (options: { rid: string; project: string; sessionId?: string; json?: boolean }) => {
      try {
    ...
    async (options: {
      ...
      try {
        ...
        if (!result.gitAvailable) {
          nextActions.push(
            'git not available; scope check skipped. Cross-check the diff manually.'
          );
        }
  ...
  addJsonOption(
    ...
    try {
      const threshold =
        options.threshold !== undefined && /^\d+$/.test(options.threshold)
    ...
    async (options: {
      ...
      try {
        ...
        if (options.json) {
          printResult(
            io,
            ok('scan.api-surface', { markdown: md, report }, [], nextActions),
            options.json
          );
        } else {
    ...
    async (options: {
      ...
      try {
        ...
        if (totalOrphans === 0) {
          ...
          nextActions.push(
            'Re-run `peaks scan orphan --project <path>` after cleanup to confirm zero.'
          );
        }
        if (options.format === 'json' && !options.json) {
          process.stdout.write(JSON.stringify(report, null, 2) + '\n');
        ...
        if (options.json) {
          printResult(
            io,
            ok('scan.orphan', { markdown: md, report }, [], nextActions),
            options.json
          );
        } else {
          process.stdout.write(md + '\n');
    ...
    async (options: {
      ...
      try {
        ...
        if (options.json) {
          printResult(
            io,
            ok('scan.karpathy', { markdown: md, report }, [], nextActions),
            options.json
          );
        } else {
          process.stdout.write(md + '\n');
