...
} from '../../services/rd-swarm/rd-swarm-service.js';
...
export async function runSwarmPlan(io: ProgramIO, options: SwarmPlanOptions): Promise<void> {
  ...
  if (options.dryRun === false) {
    printResult(
      io,
      fail('swarm.plan', 'UNSUPPORTED_NON_DRY_RUN', 'Only dry-run planning is supported', {}, [
  ...
  if (
    ...
    !['full-auto', 'guided', 'rnd'].includes(options.codeMode)
  ) {
    printResult(
      io,
      fail('swarm.plan', 'UNSUPPORTED_CODE_MODE', `Unsupported code mode ${options.codeMode}`, {}, [
  ...
  try {
    ...
    if (!plan.available) {
      ...
      printResult(
        io,
        fail('swarm.plan', code, 'RD swarm planning is blocked', plan, plan.nextActions),
        options.json
      );
      process.exitCode = 1;
      return;
...
export function registerSwarmCommands(program: Command, io: ProgramIO): void {
  ...
  if (swarm) {
    addSwarmPlanChangeIdOptions(swarm.command('plan-change-id')).action(
      (options: SwarmPlanOptions) => runSwarmPlan(io, options)
    );
  }
  addSwarmPlanChangeIdOptions(program.command('swarm-plan-change-id')).action(
    (options: SwarmPlanOptions) => runSwarmPlan(io, options)
  );
}
