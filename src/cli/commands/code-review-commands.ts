...
export function registerCodeReviewCommands(program: Command, io: ProgramIO): void {
  const codeReview = program
    ...
    .description(
      'Code-review primitives for peaks-rd Gate B3. ECC bridge (JS/TS) + OCR 1.8.x multi-language.'
    );
  ...
  addJsonOption(
    codeReview
      ...
      .description(
        'Read-only probe: returns the 5-state OCR 1.8.x runtime envelope (ready / ocr18-missing / binary-missing / llm-config-missing / detection-failed).'
      )
    ...
    try {
      ...
      const envelope =
        result.state === 'ready'
          ...
          : fail(
              'code-review.detect-ocr-18',
              result.state.toUpperCase().replace(/-/g, '_'),
              `ocr 1.8.x is not ready: ${result.state}`,
              result,
              [...result.nextActions]
            );
      ...
      printResult(
        ...
        fail(
          ...
          ['Re-run with a valid npx on PATH.']
        ),
        options.json
      );
    }
    ...
    try {
      ...
      if (!outcome.ok) {
        printResult(
          ...
          fail(
            ...
            outcome.code,
            outcome.message,
            { state: 'acquire-failed', shell: outcome.shell.kind, shellNote: outcome.shell.note },
            [ACQUIRE_RETRY_ACTION]
          ),
          options.json
        );
        process.exitCode = 1;
      ...
      if (after.state !== 'ready') {
        printResult(
          ...
          fail(
            command,
            'OCR18_ACQUIRE_INCOMPLETE',
            `the acquisition exited 0 but ${OCR_18_PACKAGE} still does not resolve (${after.state})`,
            { state: after.state, shell: outcome.shell.kind, shellNote: outcome.shell.note },
            [...after.nextActions]
          ),
          options.json
        );
      ...
      printResult(
        ...
        ok(
          command,
          {
            ...
            shell: outcome.shell.kind,
            shellNote: outcome.shell.note,
      ...
      printResult(
        io,
        fail(command, 'OCR18_ACQUIRE_FAILED', getErrorMessage(error), { state: 'acquire-failed' }, [
          ACQUIRE_RETRY_ACTION
        ]),
        options.json
      );
  ...
  addJsonOption(
    codeReview
      .command('run-ocr-18')
      .description(
        'Invoke `ocr review --filter-language <lang>` (or `ocr delegate preview` when --delegate) for one of 8 supported languages.'
      )
    ...
    if (language === undefined || !SUPPORTED_LANGUAGES_SET.has(language)) {
      printResult(
        io,
        fail(
          'code-review.run-ocr-18',
          'LANGUAGE_UNSUPPORTED',
          `unsupported language: ${language ?? '<missing>'}`,
          { state: 'language-unsupported', supported: OCR_18_LANGUAGES },
          [`Pick one of: ${OCR_18_LANGUAGES.join(', ')}`]
        ),
        options.json
      );
      return;
    ...
    const envelope =
      result.state === 'ok'
        ...
        : fail(
            'code-review.run-ocr-18',
            result.state.toUpperCase().replace(/-/g, '_'),
            `ocr 1.8.x state: ${result.state}`,
            result,
            ['Re-run `peaks code-review detect-ocr-18 --json` to refresh.']
          );
    printResult(io, envelope, options.json);
  });
  ...
  addJsonOption(
    ...
    const envelope =
      result.state === 'ok'
        ...
        : fail(
            'code-review.ocr-18-delegate-preview',
            result.state.toUpperCase().replace(/-/g, '_'),
            `ocr 1.8.x state: ${result.state}`,
            result,
            ['Re-run `peaks code-review detect-ocr-18 --json` to refresh.']
