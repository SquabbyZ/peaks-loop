...
export function registerEvolutionCommands(program: Command, io: ProgramIO): void {
    ...
    (options: {
      ...
      try {
            ...
            ok(
              'evolution.propose',
              {
                proposal: {
                  ...
                  ...(options.briefPointer !== undefined
                    ? { brief_pointer: options.briefPointer }
                    : {})
                },
                nextActions: [
        ...
        if (err instanceof EvolutionIntegrityError) {
          printResult(
            io,
            fail('evolution.propose', err.code, err.message, { findings: err.findings }, [
    ...
    async (options: {
      ...
      try {
        ...
        if (err instanceof EvolutionIntegrityError) {
          printResult(
            io,
            fail('evolution.evaluate', err.code, err.message, { findings: err.findings }, [
  ...
  addJsonOption(
    ...
    (options: {
      ...
      try {
        ...
        try {
          const svc = new EvolutionService(db);
          const updated = svc.revert(options.proposal, options.userConfirmation);
          if (!updated) {
            printResult(
              io,
              fail(
  ...
  addJsonOption(
    evolution
      ...
      .option('--project <path>', 'project root (default: cwd)')
  ).action((options: { target: string; project?: string; json?: boolean }) => {
    try {
      const target = parseTargetFlag(options.target);
      if (!target) {
        printResult(
