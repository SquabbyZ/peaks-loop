...
function registerRouted(program: Command, io: ProgramIO, name: string, argument: string): void {
  program
    ...
    .description(`Route a natural-language ${argument} to the appropriate Peaks skill`)
    .action((value: string) => {
      ...
      if (!input) {
        io.stdout(
          JSON.stringify({
            ...
            command: `super.${name}`,
            code: 'MISSING_ARG',
            message: `${argument} must not be blank`,
            data: {},
            warnings: [],
            nextActions: []
          })
...
function registerAsk(program: Command, io: ProgramIO): void {
  program
    ...
    .description('Route a free-form question to the Peaks solo dispatcher')
    .action((question: string) => {
      ...
      io.stdout(
        envelope('super.ask', {
          ...
          confidence: 1,
          alternatives: [],
          rationale: 'Free-form question delegated to the Peaks solo dispatcher',
          nextActions: ['LLM: dispatch peaks-solo with this question']
        })
      );
    });
}
