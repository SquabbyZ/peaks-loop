/**
 * peaks perf-audit * CLI surface — independent performance audit skill
 * driver. Per slice v2.12.0 (Group A, Tier 3). Symmetric to
 * `security-audit-commands.ts`.
 *
 * Subcommands:
 *   - `peaks perf-audit detect` — JSON envelope describing the
 *     5-state perf-audit runtime (ready / handoff-missing /
 *     template-missing / dispatch-failed / envelope-malformed).
 *   - `peaks perf-audit run` — runs the full pipeline:
 *     detect → read handoff + template → write audit artifact to
 *     `.peaks/_runtime/<sid>/audit/perf-<rid>.md`. Reads the
 *     envelope JSON from --envelope file or stdin.
 *
 * The actual LLM-in-loop judgement is NOT in this CLI — the parent
 * LLM (peaks-perf-audit skill prompt) is the judgement core.
...
export function registerPerfAuditCommands(program: Command, io: ProgramIO): void {
  ...
  addJsonOption(
    perfAudit
      .command('detect')
      .description(
        'Read-only probe: returns the 5-state perf-audit runtime as a JSON envelope (ready / handoff-missing / template-missing / dispatch-failed / envelope-malformed). The peaks-perf-audit skill calls this first to decide whether to invoke `run`. Mirrors `peaks security-audit detect` semantics.'
      )
  ...
  addJsonOption(
    perfAudit
      .command('run')
      .description(
        'Run the full perf-audit pipeline: detect → read handoff + template → write audit artifact to `.peaks/_runtime/<sid>/audit/perf-<rid>.md`. Reads the envelope JSON from --envelope <path> or stdin. The peaks-perf-audit skill emits the envelope; this CLI validates the shape via isPerfAuditEnvelope and writes the artifact. Exits 0 on success, 1 on any failure mode (detection, validation, or write).'
      )
      .option('--rid <rid>', 'request id (e.g. 2026-06-27-v2-12-...)')
      .option('--sid <sid>', 'session id (e.g. 2026-06-27-session-...)')
      .option('--project <path>', 'project root (default: cwd)')
    ...
    if (sid === undefined || sid.length === 0) {
      printResult(
        ...
        fail(
          'perf-audit.run',
          'SID_REQUIRED',
          'session id (--sid) is required for perf-audit.run',
          {},
    ...
    if (rid === undefined || rid.length === 0) {
      printResult(
        ...
        fail(
          'perf-audit.run',
          'RID_REQUIRED',
          'request id (--rid) is required for perf-audit.run',
          {},
    ...
    try {
      ...
      if (!isPerfAuditEnvelope(envelopeValue)) {
        printResult(
          io,
          fail(
            'perf-audit.run',
            'ENVELOPE_MALFORMED',
            'envelope failed isPerfAuditEnvelope validation (expected { verdict, violations, summary })',
      ...
      if (result.detect.state !== 'ready' || result.artifactPath === null) {
        printResult(
          ...
          fail(
            ...
            `perf-audit is not ready: ${result.detect.state}`,
            result,
