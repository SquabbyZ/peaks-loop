...
export function registerLogCommands(program: Command, io: ProgramIO): void {
  ...
  addJsonOption(
    log
      .command('tail')
      .description(
        "Print the last 50 lines of today's peaks-loop log (JSONL). Pass --lines N to change the count, --date YYYY-MM-DD to read a different day, --batch <id> to filter by batchId, --json to print a JSON envelope."
      )
      .option('--lines <n>', 'number of lines to show (default 50)', (value: string) =>
        Number(value)
      )
      .option(
        '--date <YYYY-MM-DD>',
    ...
    try {
      ...
      const safeLines =
        Number.isFinite(requestedLines) && requestedLines > 0 ? Math.floor(requestedLines) : 50;
        ...
        ok(
          'log.tail',
          {
            ...
            ...(options.batch !== undefined
              ? { batch: options.batch, batchMatches: result.batchMatches ?? 0 }
              : {}),
      ...
      printResult(
        io,
        fail('log.tail', 'LOG_TAIL_FAILED', getErrorMessage(error), {}, [
          'Check `peaks log ls` to see available log files'
        ]),
        options.json
      );
      process.exitCode = 1;
  ...
  addJsonOption(
    log.command('ls').description('List peaks-loop-*.log files in ~/.peaks/logs/ (newest first).')
