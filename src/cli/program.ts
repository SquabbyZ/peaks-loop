...
export function createProgram(
  io: ProgramIO = { stdout: defaultStdoutSink, stderr: defaultStderrSink }
): Command {
  const program = new Command();
  program
    .name('peaks')
    .description(
      `Peaks Loop ${CLI_VERSION} — loop engineering CLI: workflow primitive / loop guards / evaluators / slice orchestration
