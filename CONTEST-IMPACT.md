...
截止 v2.2.2（2026-06-14）。

> ⚠️ **本文档已过期（2026-09-15 标注，原文保留不删改）。**
>
> 原文此处写的是"**所有数字与代码路径都可复核**"。2026-09-15 的实际核查结果：
>
> | 原文声明 | 实测 |
> |---|---|
> | 冻结于 v2.2.2（2026-06-14） | 当前 **v4.0.49** —— 落后约 **100 个版本 / 3 个月** |
> | `peaks-code-resume`（第 11、18 行） | 该技能**已重命名为 `peaks-resume`**，全仓仅本文档与一条 2026-06 的历史记忆提及 |
...
· 新成员 30 秒上手：新人读 5-10 份文档重建上下文平均 2-4 小时/人；peaks-code 覆盖 ≥ 90% 场景 + peaks-code-resume 跳过已完成 gate——效率 ↑ ≈ 99%。
...
· 续接场景 token 省 60-80%：续接切片，agent 重读 + 重建上下文，3-5k token；peaks-code-resume 跳过已完成 gate。
...
· 多终端并行：3-4 个终端同时挂 peaks-code，各跑不同 PRD，session 独立绑定。一人撑起一个小团队吞吐量——白天主线，夜里 sub-agent 跑回归。
· 更高的代码质量：peaks-sop 兜住 TODO / 漏测试；peaks-qa 跑三关。省 review + 省返工。
· 精力放回业务 / 体验打磨：写脚手架 / 跑测试 / 改 lint 占 50%，被 peaks-code 跑完——省下的时间投到"用户看了什么反应"、"这一步他会不会卡"。
· 学习预算：peaks-code 在终端跑 routine，我读 Context7 / GitHub / arxiv；看中了，30 分钟 peaks-sc 评估 + peaks-sop 落 SOP + peaks-hooks 装上——前沿从"看新闻"变"周级别落地"。
